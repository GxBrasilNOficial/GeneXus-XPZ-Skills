...
- [ ] When `New-XpzImportPackage.ps1`/`.py` was used, the 9-FD front-acervo drift gate (`Test-GeneXusFrontAcervoDrift.ps1`) ran before packaging (it always runs, fail-closed: `-AcervoPath` explicit or canonical `<RepoRoot>/ObjetosDaKbEmXml` auto-resolved, reported in `acervoResolvedBy`; no acervo resolvable blocks packaging) and returned `pass` or all findings were resolved (for `fail` findings — `front-older-than-acervo` or `front-object-type-drift` — packaging was blocked; for `warn` findings — `front-equals-acervo` or `lastupdate-unparseable` — each was explicitly acknowledged or resolved before continuing)
