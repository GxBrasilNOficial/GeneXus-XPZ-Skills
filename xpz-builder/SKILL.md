...
9-FD. Front-Acervo drift gate — run before any packaging when `ObjetosDaKbEmXml` exists as the official corpus:
   ...
   - `fail` (any `fail` finding) → **ABORT** packaging; report each finding and the corrective action:
      ...
      - `front-object-type-drift`: the front XML and official corpus XML have the same `guid` but different `Object/@type` — **ABORT** and ask for a human decision. Do not auto-copy from the corpus or bump `lastUpdate`; first determine whether the correct object is the front XML, the corpus XML, or whether the generated/selected XML is contaminated.
