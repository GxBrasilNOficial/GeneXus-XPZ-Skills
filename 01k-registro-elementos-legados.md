...
## Limitação conhecida (materialização cross-fluxo moderno↔legado)

O `Sync-GeneXusXpzToXml.ps1` materializa cada item em `<FolderType>/<NormalizedName>.xml` e o `Write-ItemToDestination` **não** olha o `dataSource`: a identidade de acervo é apenas `FolderType|NormalizedName`. Logo, se o **mesmo** objeto for materializado primeiro por um export **moderno** e depois por um export **legado GeneXus 9** (ou vice-versa) na mesma pasta/nome, o segundo **sobrescreve** o primeiro silenciosamente, ainda que os envelopes difiram (`dataSource="gx-legacy-export"` + `GxLegacyPayload` + `guid=""` no legado vs envelope moderno). É **raro** (uma KB real costuma ser moderna **ou** legada, não as duas no mesmo acervo) e foi **deferido por design** na revisão por pares Fase 2 (manuscrito §9: documentar, não tratar). Distinto do fail-closed de pacote **misto**, que é dentro de um único `ExportFile`; esta colisão é **entre execuções de sync**. Acompanhamento em `999-ideias-pendentes.md`.
