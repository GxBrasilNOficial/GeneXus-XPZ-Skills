...
- **Risco residual (Decisão G):** um XML existente **não-parseável** é tratado como `dataSource=""` (moderno) e **não bloqueia** a escrita, nem sob `-BlockCrossFlowDataSource` (falha seguro); a detecção emite só um warning.
