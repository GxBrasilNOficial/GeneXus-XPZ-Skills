...
- `Invoke-Antigravity.ps1 [-Message <prompt> | -MessagePath <arquivo>] [-Model <m>] [-Profile public-review] [-Mode plan] [-ReceiptPath <json>] [-AntigravityExe <path>] [-TimeoutSec <1-3600>]` — síncrono no perfil fixo **`public-review`** (prompt → texto do envelope JSON `.response`). Cria scratch cwd curto, novo, vazio e descartável; recusa `.git`, reparse point, raiz/ancestral de repositório ou de KB paralela; não aceita `-Cd` no perfil. Redireciona `USERPROFILE`/`HOME`/`APPDATA`/`LOCALAPPDATA`/`TEMP`/`TMP`/`XDG_*`/`ANTIGRAVITY_HOME`/`AGY_HOME` somente no processo filho. Isso é **higiene de estado**, não sandbox: o keyring de credenciais continua global, e caminhos absolutos/MCP/navegador/rede não ficam isolados. Usa apenas `--mode plan`, `--output-format json`, `--print-timeout` e `--model`; `-MessagePath` é exclusivo com `-Message`, mas continua argument-based, com omissão fail-closed acima de 30000 chars, sem truncar. O recibo `antigravity-public-review-receipt` schema 1 registra `cliVersion`, postura, scratch e limpeza `clean|degraded|failed`; mudança de versão frente à baseline promovida exige nova sonda/smoke antes de promover evidência. `-ScratchPath` e `-SimulateCleanupFailure` são seams **somente de self-test** e ficam bloqueados pelo dispatcher. Em falha tipada, o adapter classifica `Reason=quota` / `unauthenticated` a partir do **raw** `stderr+stdout` (nunca a partir só do extrator de linhas); em timeout, mata descendentes, espera o runner flushar e pode promover cota/auth pelo residual.
- `AntigravityCliSupport.ps1` (dot-source) — descoberta **fail-closed** do `agy.exe` (no `PATH` ou `%LOCALAPPDATA%\agy\bin\agy.exe`), validação de contrato **mínimo de descoberta** (`--print`/`--prompt` e `--mode`), leitura de `agy --version` para o recibo, `Resolve-AntigravityPublicReviewFailureReason` (auth vence cota; `ErrorField` genérico não esconde auth no raw) e Detail em janela ≤400 chars centrada no match. O `$quotaFailurePattern` do support é **propositalmente diferente** do dispatcher — **não** há ordem total: mais estrito em `\bquota\b` / `\b402\b`/`\b429\b` e sem `exhausted` solto; mais permissivo em `rate\s*limit` / `too\s*many\s*requests` (casa variantes que o literal do dispatcher não casa). **Não** afirmar paridade de regex. `$DispatcherQuotaProbePattern` é cópia literal do padrão do dispatcher e serve **só** para sanitizar Detail de auth (evitar realimentar falso-cota no painel). Risco residual aceito: eco do prompt no stderr com tokens de cota pode produzir falso `Quota(Raw)`; como auth **vence** e varre o mesmo raw amplo (`authenticat*`/`authoriz*`/`sign in`/`login`), eco do prompt com esses termos também pode produzir falso `Unauthenticated(Raw)` (o painel projeta como `error` e esse estado vence a cota). Validação: `Test-AntigravityCliSupportSelfTest.ps1` e `Test-AntigravityPublicReviewSelfTest.ps1` (fake-exe, sem rede).
...
inválido, autenticação com exit 0/≠0, `status != SUCCESS`, cota tipada (`Individual quota reached`) e timeout com residual de cota.

Em falha, o mesmo envelope traz `status="ERROR"`, `response=""` e `error` com o motivo — e o processo sai com **exit code ≠ 0**. O caminho de erro observado passa por `Resolve-AntigravityPublicReviewFailureReason` sobre o raw (com `Normalize-AntigravityErrorField` quando há `.error`); o extrator `Get-AntigravityErrorMessage` só entra no Detail do ramo sem classificação (`processFailure`). O guard de `status != SUCCESS` no adapter é **defesa em profundidade** para um exit 0 com status de erro, não o fluxo observado. Campos além de `status`/`response`/`error` não são consumidos pelo adapter.
...
`-TimeoutSec` (default 1200s, repassado ao `--print-timeout` do CLI) antes de usá-lo em painel.
...
2. **Curadoria de revisores preferidos**: oferecer salvar **a seleção que o usuário já fez** via
   `Set-LlmDelegatePreferredReviewers.ps1 -Orchestrator <harness> -Scope …`
   (obrigatórios no corpo; ausência → exit 1). **Regra de escopo (edição nesta sessão):**
   1. Rodar `Resolve-… -Orchestrator <harness>` e ler `preferenceSource` /
      `effectivePreferredPath` / `cascadeOrchestratorExists` / `cascadeMachineExists`.
   2. Se o pedido for a lista preferida **deste harness** (padrão quando o usuário não diz
      «lista da máquina» / «todos os harnesses»):
      - se `preferenceSource=orchestrator` **ou** `cascadeOrchestratorExists=true` →
        `-Scope orchestrator` (editar o ficheiro que o Resolve já lê);
      - se só existe machine (`preferenceSource=machine` ou só `cascadeMachineExists`) →
        `-Scope machine` **ou** criar orchestrator se o usuário quiser lista só deste harness;
   3. `-Scope machine` com ficheiro do orquestrador presente só com pedido **explícito** de
      lista machine; o `Set-` emite `harnessResolveReadsThisPath=false` e
      `diagnostics` contendo `machine-write-shadowed-by-orchestrator-file` — **mostrar isso
      antes/depois** e não tratar a gravação como «lista do Cursor/Claude/… atualizada».
   4. Recibo mínimo antes de gravar: path efetivo, `preferenceSource`, ranks atuais, e se
      machine e orchestrator divergirem, dizer qual muda e qual continua a mandar neste harness.
   O path efetivo é o da cascata schema 3 sob `%LOCALAPPDATA%\xpz-llm-delegate\` — **não** citar
   um path fixo como única opção; a fonte de verdade é o envelope do `Resolve-`.
