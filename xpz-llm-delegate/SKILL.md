...
- **Preparação de artefatos:** o harness aceita `-ManuscriptPath` legado ou `-ManuscriptText`. Com `-ManuscriptText`, antes de qualquer gate/despacho ele chama `New-LlmDelegatePeerReviewArtifacts.ps1`, que publica `manuscript.md`, `reviewers.json` e `preparation-manifest.json` sob `<TempDir>/<RoundId>`; falha nessa etapa termina com resumo estruturado do próprio dispatcher (`roundStarted=false`, `dispatchStarted=false`, zero revisores despachados).
