...
- `ClaudeCodeCliSupport.ps1` (dot-source) — descoberta **fail-closed** do `claude.exe`, validação de versão/flags mínimas, extração de erros e classificação heurística de `workspace-not-trusted` a partir de `stderr`.
...
- Se o `stderr` do Claude Code casar o detector **heurístico** de workspace ainda não confiável,
  ...
  houver. A mensagem real da CLI ainda precisa de captura empírica; não marcar confiança automaticamente:
  o humano deve abrir o workspace no Claude Code e confirmar a confiança, ou escolher outro backend.
