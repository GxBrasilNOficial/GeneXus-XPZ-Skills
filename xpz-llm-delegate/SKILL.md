...
- `CodexCliSupport.ps1` (dot-source) — descoberta **fail-closed** do `codex.exe`; `Resolve-CodexJobTempDir`; `Resolve-CodexPidAndStartTime` (merge campo-a-campo, nunca lança 22); KeepDays por classe A/B/C; env `XPZ_CODEX_DISABLE_KEEPDAYS=1` desliga a limpeza (`Invoke-CodexJobsKeepDaysCleanup` retorna imediato — caminho de produção, usado também por self-test/painel isolados). Self-test: `Test-CodexCliSupportSelfTest.ps1`; `Test-CodexDurableCaptureSelfTest.ps1`.
