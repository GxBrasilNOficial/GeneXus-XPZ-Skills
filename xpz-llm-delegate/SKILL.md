...
- **opencode só-`public` no v1:** opencode em `kb-sensitive` → `unavailable` (sem gate/despacho). O confinamento do agente custom **está ATIVO** (default `reviewer-ro` sem execução/escrita, `external_directory[*]=deny` para leitura fora do cwd herdado, com exceções internas do opencode observadas nos fixtures 1.17.20); liberar opencode em `kb-sensitive`/pasta paralela + mecanizar cwd-seguro ficou **ADIADO** (`999-ideias-pendentes.md`, eixo de leitura). O recorte `.env` dentro do cwd é **urgente**: o OpenCode 1.17.20 traz `read "*.env" -> ask`, mas o bloco posterior do `reviewer-ro` tende a reabrir `read "*" -> allow`; não usar cwd com `.env` real até a frente ser resolvida. O adapter opencode **não** tem `-Cd` (por isso nunca o recebe; a leitura fica confinada ao cwd HERDADO); os demais (codex/claude-code/gemini/copilot) recebem `-Cd` por precedência (explícito → `ParallelKbRoot` em `kb-sensitive` → cwd em `public`) com **fail-closed** quando `kb-sensitive` e faltam ambos.
...
- **Eixo de LEITURA — premissa INVERTIDA (medido em opencode 1.17.20).** A doc anterior afirmava que a
  ...
  padrão `external_directory[*]` fica bloqueado, garantido e independente do modo; fixtures 1.17.20
