...
- `Set-LlmDelegatePreferredReviewers.ps1 -ReviewersJson <json> -Orchestrator <cursor|claude-code|codex|opencode> -Scope <machine|orchestrator> [-PreferredRoot <dir>] [-OutputPath <json>] [-Preview] [-Overwrite]` — persiste curadoria **schema 3** em cascata (`preferred-reviewers.<orchestrator>.json` ou `preferred-reviewers.json` machine) ou desvio `-OutputPath`. Titulares `delegation-cli` (6 backends CLI) ou `orchestrator-native-subagent` (`backend=orchestrator-native`; nativo proíbe `fallbackChain[]`); `rank`, `reasoningEffort`, `invokeArgs` obrigatório; `harnessModelId` obrigatório no nativo; chave de harness Cursor nas duas grafias (`cursor/<modelo>` e `cursor-<modelo>`) só é aceita como titular **nativo** e com Criador conhecido (`cursor/grok-*` → `xai`, `cursor/composer-*` → `anysphere`) — como alvo CLI ou elo de `fallbackChain[]` é bloqueada (`native-cursor-prefix`, exit 3), porque nenhum backend CLI despacha para modelo que só existe dentro do Cursor; sanitização `invokeArgs` (`backend`/`model`/`profile`/`oss`/`localProvider`/`timeoutSec`; opencode=`model`=chave completa; demais CLI=último segmento); **nunca** token/baseURL/header/path. Backend desconhecido → throw tipado (`reason` nu), sem `skipped[]`. `-Preview` valida sem gravar; `-Overwrite` sobre destino schema 3 válido.
