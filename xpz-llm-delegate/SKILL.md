...
- `Invoke-Codex.ps1 [-Message <prompt> | -MessagePath <arquivo>] [-Model <m>] [-Oss] [-LocalProvider <ollama|lmstudio>] [-Profile <id>] [-Cd <dir>] [-CodexExe <path>] [-TimeoutSec <s>] [-TempDir <path>] [-KeepDays <n>] [-RetentionMode public|kb-sensitive]` — síncrono (prompt → texto). Prompt via stdin; resposta final pelo `output-last-message` persistido em `<TempDir>\<GUID>.lastmsg.txt` (cascata TempDir: Bound → env `XPZ_CODEX_JOBS_DIR` → `%TEMP%\codex-jobs`; `-TempDir` **sem** default no `param()`). `request.json` `invoke-sync` **sem** prompt; campo `captureOutcome` (`success`|`timeout`|`error`|`empty`). Timeout **sempre** lança (`excedeu` + `foi encerrado`) mesmo com bytes no lastmsg. Exit ≠ 0 com texto não-branco → devolve o parecer (`success`). Sentinelas `XPZ_CODEX_LASTMSG=` / `XPZ_CODEX_REQUEST=` via `[Console]::Error.WriteLine` no `finally` (sucesso e falha); stdout de sucesso = **só** o parecer. Em `kb-sensitive`, lastmsg só entra no throw em **falha** de execução (antes do delete); sucesso apaga lastmsg/invoke-* sem copiar. Throw/return do parecer **só** depois do `try/finally`. `-MessagePath` exclusivo com `-Message`; stdin-based, **não** sujeito ao teto ~32KB. Default `-TimeoutSec` 180 (não é teto). Chamadores `2>&1` humanos: risco aceite (sentinela no sucesso mistura-se ao stdout agregado).
...
- `Watch-CodexJob.ps1 -JobId <guidN> -ProcessId <pid> [-TempDir <path>] [-ExpectedStartTimeUtc <fffZ>] [-IntervalSeconds <1-30>] [-SilenceThresholdSeconds <30-3600>]` — monitor; códigos `{0,21,22,99}`; JobId GUID N **antes** de TempDir; ProcessId Mandatory. Árvore: `result.json` existe → 21; alvo vivo → identidade (hora omitida/ilegível → 22); alvo morto sem grupo → AVISO exit 0 sem `result.json`; morto+hora+grupo → promove shape 8 campos com `finishedAt` UTC `fffZ`. 22/99 **não** gravam `result.json`. Sem `finally` de gravação. Exit 22 simétrico ao 21 (`BLOCK: identidade do processo recusada.`).
- `CodexCliSupport.ps1` (dot-source) — descoberta **fail-closed** do `codex.exe`; `Resolve-CodexJobTempDir`; `Resolve-CodexPidAndStartTime` (merge campo-a-campo, nunca lança 22); KeepDays por classe A/B/C. Self-test: `Test-CodexCliSupportSelfTest.ps1`; `Test-CodexDurableCaptureSelfTest.ps1`.
