...
- **Importância** — baixa (caso raro, limite conhecido declarado; sem perda silenciosa no fluxo normal de uma única KB). `scripts/Sync-GeneXusXpzToXml.ps1` materializa cada item em `<FolderType>/<NormalizedName>.xml` e o `Write-ItemToDestination` **ignora o `dataSource`**: se o **mesmo** objeto for materializado primeiro por um export **moderno** e depois por um export **legado GeneXus 9** (ou vice-versa) caindo na mesma pasta/nome, o segundo **sobrescreve** o primeiro sem aviso — os envelopes diferem (`dataSource="gx-legacy-export"` + `GxLegacyPayload` + `guid=""` no legado vs envelope moderno), mas a identidade de acervo é só `FolderType|NormalizedName`. Manuscrito Fase 2 §9 decidiu **documentar e não tratar nesta frente** (raro: uma KB real é moderna **ou** legada, não as duas no mesmo acervo). Documentado no `01k` («Limitação conhecida»).
- **Maturidade** — ideia (decisões de design em aberto). Direção possível: detecção de colisão de `dataSource` divergente para o mesmo `FolderType|NormalizedName` (warning fail-soft no JSON, ou bloqueio opt-in), sem misturar com o fail-closed de pacote **misto** (que é dentro de um único `ExportFile`; esta colisão é **entre execuções**). Gatilho real: um acervo que de fato receba os dois fluxos. Derivada do fechamento do **Motor de export legado GeneXus 9** (migrado para `historico/IdeiasImplementadas_202606.md`).
