...
- **Importância** — média (gap real com workaround manual; **escala para alta** assim que uma KB Java for aferida em produção). **Dor original (motivadora):** `deployment_hosting_kind` só aceitava `dotnet-core-self-host`/`dotnet-framework-iis` e o gate de deploy-bin é .NET por dentro (`web\bin\*.dll`+`*.config`+sentinela `GxNetCoreStartup.dll`); uma KB Java/Tomcat não era representável (o usuário contornava forjando metadata .NET) nem verificável (o gate sairia procurando `.dll`/`GxNetCoreStartup.dll` numa app Java — falso-negativo de segurança, verde enganoso). **Pós-Fase 2:** `java-tomcat` agora é **representável** (aceito pelo registro via `Set-XpzKbSourceMetadataDeployment.ps1`, não mais `[ValidateSet]` .NET-only) e o falso-negativo virou **skip explícito** (`skipped-hosting-unsupported`/exit 0 — skip ≠ deploy validado). **Pós-Fase 3** (2026-07-05): a **verificação real por família** existe — o Eixo A roda um co-gate Java (publicação atestada por conjunto de artefatos no `WEB-INF\classes` externo). **Pós-Fase 5** (2026-07-07): o aterramento empírico foi registrado para os ambientes disponíveis. **Pós-assistente opt-in** (2026-07-08): o setup ganhou assistente read-only para sugerir metadata Java a partir de `model.ini`/`gradle.properties` com validação de topologia/sentinelas/pacote e confirmação explícita antes de gravação. **Fechamento formal v1 prática** (2026-07-08): metadata/setup + deploy-bin Java/Tomcat estão documentados como usáveis/fail-safe; o que fica abaixo são frentes futuras, não bloqueadores da v1. Continua aberto, se algum dia houver ambiente, medir stack clássico puro Tomcat 8/9 + JDK 8. Origem: proposta de usuária de outra máquina (KB Java/Tomcat), 2026-07-01.
- **Maturidade** — **v1 prática encerrada formalmente** (2026-07-08) para metadata/setup + deploy-bin Java/Tomcat; a entrada permanece neste `999` apenas pelos follow-ups explícitos. Fase 1 **implementada** (2026-07-02): registro + API + `ArgumentCompleter` fail-soft + self-test de drift + golden de regressão .NET; convergida por revisão por pares da implementação (v1→v9, 3 famílias, zero gaps sobre a versão final) e pela pré-push reforçada. Fase 2 (fiação do registro nos consumidores via API pública + guardas de família dos Eixos C/B) e Fase 4 (doc paridade `02`/`08`/`09`/`10`/README/SKILLs) **implementadas** (2026-07-02, `035f2c7`, **pushado**); **Fase 0 (aterramento empírico) concluída sem gap** (2026-07-04): Q5b **confirmado** pelo re-teste decisivo (rota manual IDE, sem import) — o sinal de frescor Java é o `<obj>_impl.class` sob o `WEB-INF\classes` **externo**, aferível **por mtime** (build com mudança regrava e publica; sem mudança não toca) — ver [`evidence-catalog-java-tomcat.md`](evidence-catalog-java-tomcat.md); **Plano B acionado** (topologia Java/Tomcat = webapp desdobrado **externo** à árvore de output, aferido na KB `EBTECH` GX18U14, sabor Jakarta, na máquina de uma colega — a máquina de dev não tem licença Java). **Fase 3 (motor do gate por família, Eixo A) IMPLEMENTADA** (2026-07-05): replano **congelado** (v1→v10, 5 famílias, CONGELA/APROVA unânime) persistido em [`java-tomcat-fase3-replano.md`](java-tomcat-fase3-replano.md) + [`java-tomcat-fase3-checklist-implementacao.md`](java-tomcat-fase3-checklist-implementacao.md); `java-tomcat` Eixo A `supported` (co-gate de 4 quadrantes por conjunto de artefatos + skew bidirecional sobre o `WEB-INF\classes` externo; nunca falso-`fresh`); split de estado per-eixo; §9 allowlist-invertida; regressão .NET provada por golden byte-a-byte; novo self-test do co-gate. **Fase 5 empírica registrada** (2026-07-07): fechou Jakarta para `Rebuild All`→`Lf=∅,Pf≠∅`, latência/âncora de cópia, ator `copyTomcat*` e sufixos; mediu também o environment GeneXus `JAVA_EE`/Gradle `javaEE` disponível com sinais reais `javax`, mas no stack Tomcat 11/JDK 21/Servlet 6 e com jars `jakarta.*` — **não** foi medido Java EE clássico puro Tomcat 8/9 + JDK 8. **Assistente opt-in de metadata implementado** (2026-07-08): `Resolve-XpzJavaTomcatMetadataSuggestion.ps1` sugere os campos Java, valida `model.ini`/`gradle.properties` contra topologia/sentinelas/pacote e deixa a escrita para confirmação explícita via `Set-XpzKbSourceMetadataDeployment.ps1`. **Fica para frentes futuras/paridade TOTAL:** (1) **Pós-v1 — os MOTORES dos Eixos B (fonte gerado `.java`) e C (runtime-freshness `.java`)**, que hoje só têm guarda-de-família/skip para KB Java (ver a fase «Pós-v1» no design congelado); (2) eventual automação além do assistente read-only, se algum dia for aprovada sem perder a exigência de confirmação explícita. **Handoff de retomada versionado (qualquer agente):** [`historico/PROMPT-retomada-java-tomcat.md`](historico/PROMPT-retomada-java-tomcat.md). Design **congelado** após revisão por pares de 6 rodadas (v1→v6; 3 famílias) + replano de 10 rodadas (Plano B, topologia externa); a Fase 0 confirmou a ramificação Plano B já prevista no congelamento.
- **Fase 5 parcial (2026-07-07, KB `EBTECH`, Jakarta/Tomcat 11/JDK 21):** coleta empírica recebida e registrada em [`evidence-catalog-java-tomcat.md`](evidence-catalog-java-tomcat.md). Achados: `Rebuild All`/rodada observada **não** produziu `Lf=∅, Pf≠∅` (`Lf=379`, `Pf=624`, `Pf\Lf=245`, `Lf\Pf=0`), mas a pergunta não fica fechada como rodada dedicada porque a coleta reutilizou rodada existente e o wrapper reportou falha operacional por log bloqueado; latência parcial = ~61s de fim de `compileJava` ao último `.class` publicado e ~1,6s dentro da janela de mtimes publicados; ator da cópia confirmado como Gradle chamado pelo Build All GeneXus/MSBuild (`copyTomcat*`); `javax` segue **não medido**; sufixos observados por stem/stub-base = `<base>`, `_impl`, `__default`, `__gam`, com stems derivados por objeto GeneXus (`_bc`, `ww*`, `RESTInterfaceIN/OUT`, `services_rest`, `Sdt*`, `StructSdt*`). Achado crítico para setup: `SERVLET_DIR` do `model.ini` divergiu da publicação real observada; o assistente read-only de metadata Java endereça esse risco validando topologia/sentinela e exigindo auditoria em divergência, e qualquer automação futura não pode copiar cegamente o `model.ini`.
- **Fase 5 rodada 2 (2026-07-07, KB `EBTECH`, Jakarta/Tomcat 11/JDK 21):** nova coleta dedicada **supersede** os pontos inconclusivos da parcial, deixando o JavaEE/`javax` para a rodada seguinte. Clean build dedicado confirmou `Rebuild All -> Lf=∅, Pf≠∅` (`Lf=0`, `Pf=624`, `Pf\Lf=624`, `Lf\Pf=0`) em três rodadas `exit=0`/`BUILD SUCCESSFUL`. Latência instrumentada: `compileJavaEndAt -> lastPublishedClassMTime = 24,045s`, `copyTomcatClassesStartAt -> lastPublishedClassMTime = 1,824s`; logo `deployBinTimeSlackSeconds=5` cobre a janela interna de cópia, mas não o fim de compilação como âncora — o marco útil é fim da publicação/cópia (`copyTomcatClasses`/`copyTomcat*`, futuro `DeployStepCompletedAt`). O `SERVLET_DIR` divergente foi explicado por bloco/environment: `gradle.properties` do web output trouxe `TOMCAT_WEBAPP_PATH=...\EBTECHPrototipo_18U14`, caminho com `WEB-INF\classes` e `WEB-INF\lib\GeneXus.jar`; setup futuro deve resolver o environment ativo, validar sentinelas/pacote da app e confrontar `model.ini` com `gradle.properties` quando existir. Sufixos/stems: manter strip fechado em `{_impl,__default,__gam}`; `_bc`, `ww*`, `Sdt*`, `StructSdt*` e `StructSdtCol*` ficam stems próprios. **Aberto à parte:** stack clássico puro Tomcat 8/9 + JDK 8, se algum dia houver ambiente disponível para medir.
- **Fase 5 JavaEE/javax (2026-07-07, KB `EBTECH`, environment `JavaEE`):** nova coleta mediu a opção GeneXus `JAVA_EE`/Gradle `javaEE`, publicada em Tomcat 11/JDK 21/Servlet 6, com sinais reais `javax` (`web.xml` Java EE 3.0, `javax.ws.rs.Application`, `gxwrapperjavax-4.10.2.jar`, `javax.servlet-api-3.1.0.jar`, `javax.ws.rs-api-2.1.jar`) e também jars `jakarta.*` — portanto **não** é prova de Java EE clássico puro Tomcat 8/9 + JDK 8. O co-gate Lf/Pf validou o environment medido: `Lf=618`, `Pf=618`, `Pf-Lf=0`, `Lf-Pf=0`; `WEB-INF\classes`, `WEB-INF\web.xml`, `WEB-INF\lib\GeneXus.jar` e pacote `com\ebtech` estavam presentes. Build/publicação concluíram (`MSBuild exit=0`, `BUILD SUCCESSFUL`, `__BUILDALL_DONE__=true`) com ressalvas (142 warnings, stderr não vazio, acessos negados em arquivos da instalação GeneXus e falha interna de pós-processamento do wrapper). **Ressalva remanescente:** stack clássico puro Tomcat 8/9 + JDK 8 não medido.
- **Eixos B/C fonte Java e runtime-freshness (2026-07-09, KB `EBTECH` + KB descartável/de teste):** coleta read-only confirmou raiz de fonte Java por environment (`<TargetFullPath>\web\src\main\java`), uso prático de `JAVA_PACKAGE_NAME_FOLDER` e mapeamento de módulos GeneXus para subpackages. Para o Eixo C, `nav_objs.xml` existe e aponta via `ObjNavig` para XMLs de navegação/specification; experimento controlado com `WebPanel:TestWP` mostrou que `ObjStatus=genreq` permaneceu antes/depois de build com alteração e depois de build sem alteração, portanto **não** é critério isolado de freshness Java. O futuro motor Java deve cruzar `ObjNavig`, XMLs de navegação/specification e artefatos locais `.java`/`.class`/`.js`; o Eixo C atual segue `recognized-no-engine`/Pós-v1 e deve pular com `-DeploymentHostingKind java-tomcat`. Registro completo: [`evidence-catalog-java-tomcat.md`](evidence-catalog-java-tomcat.md), seção "Eixos B/C — fonte Java e runtime-freshness EBTECH (2026-07-09)".
- **Follow-up — remoção dos aliases legados de saída (migração-compat, deprecados na Fase 3):** os aliases `runsFreshnessEngine`/`freshnessSupportState`/`freshnessSkipStatus`/`unsupportedReason` do registro `GeneXusKbHostingKindSupport.ps1` persistem **1 ciclo de release** (derivados do Eixo A) para consumidores externos (wrappers de pasta paralela, fora do alcance da varredura do doc 13). No **próximo ciclo**, após confirmar que esses consumidores externos migraram aos campos per-eixo (`runsDeployBinEngine`/`runsSourceEngine`/`runsRuntimeEngine`), **remover** os 4 aliases do registro **e** a guarda de migração-compat (`alias == Eixo A`) do `Test-GeneXusKbHostingKindSupportDriftSelfTest.ps1`. Sem este item explícito é fácil esquecer a remoção (o CHANGELOG só promete "1 ciclo"). Achado da pré-push reforçada da Fase 3 (minimax-m3, 2026-07-06).

Design congelado íntegro (contrato do registro, decisões (a)–(e), fases, cláusula no-bridge, incógnitas empíricas) versionado em [`java-tomcat-paridade-gerador-design.md`](java-tomcat-paridade-gerador-design.md). Resumo: três eixos — **A** gate de deploy-bin (crítico), **B** diagnóstico de `.cs`, **C** runtime-freshness (`Test-GeneXusRuntimeFreshness`). Fonte única = registro `GeneXusKbHostingKindSupport.ps1` (acesso só via `Get-GeneXusKbHostingKindSupportRecord`); metadata declarado pelo usuário (`model.ini`/`GeneratorType` nunca autoridade); `[ValidateSet]` → validação manual + `ArgumentCompleter` fail-soft; nome `java-tomcat`; alvo de deploy in-place vs externo decidido na Fase 0; um alvo de deploy por KB no v1 (`deployment_hosting_kind` singular). **Cláusula no-bridge:** Fases 1/2 nunca iteram `publicationTargets[]` para acionar o motor — opaco a elas; a ponte registro↔motor é exclusiva da Fase 3.

Fase 1 (.NET-safe) **implementada** — não toca o motor de deploy-bin: registro + API + self-test de drift + `ArgumentCompleter`, com regressão .NET provada por golden tests. A **Fase 0 (2026-07-04) revelou topologia externa** (webapp desdobrado no Tomcat, fora da árvore de output; alvo localizável por `SERVLET_DIR` do `model.ini`; frescor = `.class` de objeto sob `WEB-INF\classes` externo; sentinela `GeneXus.jar`; exclusão por pacote, não por prefixo de jar), **acionando o Plano B**; detalhes em [`evidence-catalog-java-tomcat.md`](evidence-catalog-java-tomcat.md). A Fase 3 foi replanejada e implementada sobre essa topologia externa, e a Fase 5 empírica foi registrada para os ambientes disponíveis; novas medições continuam dependendo da KB Java externa porque a máquina de dev não tem licença Java.
...
- **Importância** — média (least-privilege **residual**; os eixos de execução/escrita e as ferramentas de rede já foram **fechados** — ver `historico/IdeiasImplementadas_202607.md`, «Least-privilege do revisor opencode»).
- **Maturidade** — pesquisa/ideia. O confinamento de leitura ao cwd **HERDADO** já está **ATIVO** (o reviewer-ro fixa `external_directory: deny`, medido em 1.4.4); falta **mecanizar cwd-seguro** e **liberar `kb-sensitive`**.
- **Já fechado (frente D-min, 2026-07-04):** default `-Agent reviewer-ro` + guard fail-closed nos adapters opencode; execução/escrita (`bash`/`edit`) e rede (`webfetch`/`websearch`/`task`) negadas; leitura confinada ao cwd herdado. Detalhes e correção de premissas (`permission: deny` ≡ `tools: false`; leitura **não** é machine-wide) no histórico.
- **Segue ADIADO:** (a) **mecanizar cwd-seguro** — o confinamento é ao cwd HERDADO, mas escolher um cwd sem segredos **não-versionados** (`.env` local, logs, cache) é responsabilidade **operacional** de quem dispara; não há BLOCK por cwd em `public`. Mecanizar = read-containment por allowlist de paths / validação do cwd antes do despacho. (b) **liberar opencode em `kb-sensitive`/pasta paralela** — hoje `unavailable` (`Invoke-LlmDelegatePanelDispatch.ps1`); depende de (a) + provisão project-local do `reviewer-ro` na pasta paralela (dono natural `xpz-kb-parallel-setup`).
