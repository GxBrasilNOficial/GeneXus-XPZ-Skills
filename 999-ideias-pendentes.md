...
Fase 1 (.NET-safe) **implementada** — não toca o motor de deploy-bin: registro + API + self-test de drift + `ArgumentCompleter`, com regressão .NET provada por golden tests. Fase 3 gated pela KB Java (refactor de motor: aridade escalar→lista, fork por família `...CoreDotNet`/`...CoreJava`, `runtimeExclusionPrefixes` do catálogo). Congelamento valia para a **hipótese in-place**; a **Fase 0 (2026-07-04) revelou topologia externa** (webapp desdobrado no Tomcat, fora da árvore de output; alvo localizável por `SERVLET_DIR` do `model.ini`; frescor = `.class` de objeto sob `WEB-INF\classes` externo; sentinela `GeneXus.jar`; exclusão por pacote, não por prefixo de jar), **acionando o Plano B** — Fase 3 a replanejar; detalhes em [`evidence-catalog-java-tomcat.md`](evidence-catalog-java-tomcat.md). A implementação da Fase 1 **passou** por revisão por pares (v1→v9, 3 famílias, zero gaps sobre a versão final) e pela pré-push reforçada.
