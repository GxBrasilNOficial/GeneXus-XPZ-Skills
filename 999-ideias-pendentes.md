...
- **Registro nos pontos fechados** do dispatcher/gate/manifesto (a lista canônica de backends ativos vive em [`xpz-llm-delegate/SKILL.md`](xpz-llm-delegate/SKILL.md) — não reenumerar nem contar aqui): `$AdapterScript`, `$ExeParam` (`'mimo' = 'MimoExe'`) e `$ContentionKeys` (`'mimo' = @('agent')`, igual ao opencode) em `scripts/Invoke-LlmDelegatePanelDispatch.ps1`; `ValidateSet` de `-Backend` em `scripts/Resolve-LlmDelegateAuthorization.ps1` (+ o `switch` de seleção do resolvedor); enumeração em `scripts/Build-LlmDelegateCapabilityManifest.ps1` (por `mimo models` ou pela config resolvida — preferir a que não dependa de rede).
