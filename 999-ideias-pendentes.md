...
- <!-- backend-parity: ignore --> **Importância** — média (maior que a do falso-positivo de «introduzido» acima, e por mecanismo **diferente**). Os adapters citados abaixo são **amostra dos arquivos afetados**, não enumeração do conjunto de backends. `Test-TokensRelated` aceita o par quando prefixo **ou** sufixo comum tem >= 4 chars. `Path`, `Name`, `List` e afins são sufixos de **tipo**, não raiz de contrato: casam qualquer par de parâmetros que termine igual, ainda que pertençam a contratos sem relação. Medido na pré-push de 2026-08-24 (intervalo do perfil Antigravity `public-review`): dos 3 pares sobreviventes, `ReceiptPath`/`MessagePath` (123 candidatas) e `ScratchPath`/`MessagePath` (122) responderam por **245 das 361** — e `-ReceiptPath`/`-ScratchPath` são exclusivos do `Invoke-Antigravity.ps1` (o segundo é TEST-ONLY), enquanto `-MessagePath` é o prompt comum aos seis adapters. Dos 24 arquivos com candidata desse par, **13 nem mencionam Antigravity/`agy`** (`Invoke-OpenCode.ps1`, `Invoke-Codex.ps1`, `Invoke-Gemini.ps1`, `Start-ClaudeCodeJob.ps1` e outros): nenhum poderia estar defasado quanto a `-ScratchPath`.
- **Risco real** — não é custo de triagem (o teto `-MaxFindings` limita o que chega ao revisor: 30 prosa + as não-prosa, que nunca truncam). É **falso negativo por diluição**: com `truncatedProseCount=328` na mesma medição, uma candidata de prosa legítima tem alta chance de cair entre as truncadas e nunca ser vista. O risco fica confinado à classe `prose` — a de menor valor acionável —, porque a disciplina de classe já protege `param-list-item`/`param-table-cell`/`command-example` do truncamento.
- **Maturidade** — ideia (direção clara, calibração em aberto). Direções possíveis: exigir **prefixo** comum (não sufixo), elevar o limiar, ou manter sufixo só quando ele não for sufixo de tipo conhecido (lista curta: `Path`, `Name`, `List`, `Json`, `Sec`). Qualquer calibração precisa ser confrontada contra o caso canônico `ObjectList`/`ObjectNames`/`ObjectGuids`, que casa por **prefixo** `Object` (6 chars) e é exatamente o que o gate existe para pegar — não pode ser perdido. Self-test já tem fixture desse caso canônico, então a regressão é detectável. Manter consultivo (`warn`).
