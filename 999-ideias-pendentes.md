...
- **Relacionada:** a entrada seguinte (migração stdin de `Invoke-Gemini`/`Invoke-Copilot` + limite de 32KB) segue aberta, dependente de assinatura para validação empírica.
...
- **Maturidade** — (b) IMPLEMENTADA 2026-06-22; resíduo (a) bloqueado por dependência externa. Direções: (a) **AINDA ABERTO** — quando houver **assinatura** de Gemini/Copilot nesta máquina, testar `gemini -p`/`copilot -p` lendo stdin e, se aceitarem, migrá-los ao padrão **stdin-based** espelhando o opencode (`Invoke-OpenCode`/`Start-OpenCodeJob`); só assim o teto ~32KB cai de fato para esses dois. (b) **guard de tamanho — IMPLEMENTADO** (2026-06-22): `$MaxArgvPromptChars = 30000` fail-closed (heurístico em chars) em `Invoke-Gemini`/`Invoke-Copilot`, recusa com `BLOCK` claro antes do `Argument list too long`; cobre `-Message` e `-MessagePath`. Migrado ao `historico/IdeiasImplementadas_202606.md`; ver `CHANGELOG`.
...
> Investigação concluída e migrada para `historico/IdeiasImplementadas_202606.md` em 2026-06-23. Truncamento das vozes coder = **não-determinismo de cauda raro** (não cota/429, não orçamento-de-passos, não propriedade fixa "coder=trunca"); corroborado por experimento controlado (12 runs, 4 modelos × 3) que reproduziu 1 truncamento real (`reason=tool-calls`, 28 `tool_use`, sem 429). Correção **implementada**: `-MaxAttempts` (retry-once) em `Invoke-OpenCode.ps1`, já no despacho do painel (`Invoke-LlmDelegatePanelDispatch.ps1`). A "variante read-only por prompt" foi **descartada como solução de truncamento** (`--agent plan` auto-aprova bash); a substância de menor-privilégio segue **aberta** abaixo nas frentes «Painel … concede execução de código…» e «Agente reviewer sem execução/escrita». Gap derivado novo (resposta `stop` quase-vazia escapa do veredito/retry) registrado em entrada própria abaixo.

## Painel de revisão por pares concede execução de código a modelo externo em headless (least-privilege do opencode)

- **Importância** — **ALTA / tratar com urgência** (segurança / least-privilege). Risco **ativo**: roda toda vez que o painel despacha uma voz opencode (em payload `public` é auto-liberado, **sem nó humano**). A mitigação concreta é a frente «Agente reviewer do opencode sem execução/escrita» abaixo (`permission: bash: deny`) — **esta entrada é o problema, a de baixo é o conserto; tratar em conjunto**.
- **Maturidade** — ideia (achado **empírico documentado**; parte já materializada como limite conhecido no `SKILL.md`).
- **Achado (empírico, opencode 2026-06, não-contratual):** em `opencode run` headless, as permissões de ferramenta do agente default `build` — **e do `--agent plan`** — são **auto-aprovadas**; o modelo executou `bash` (e a rotina pré-push real) sem gate. Logo, ao despachar a um modelo **externo** via `Invoke-OpenCode.ps1` com o agente default, o painel concede execução de comandos arbitrários na máquina. **Dois eixos:** (i) execução arbitrária local (qualquer modelo); (ii) exfiltração (agravante do externo). O gate `Resolve-LlmDelegateAuthorization.ps1` governa o **envio** do dado, **não** a capacidade de executar local — e em payload `public` cai em **`allow` automático** (caso nobre da diversidade na raiz das skills), então o risco **não passa por nó humano**; vale também para `ask` autorizado.
- **Já feito nesta frente:** seção «LIMITE CONHECIDO — OPENCODE: AGENTE DEFAULT AUTO-APROVA FERRAMENTAS LOCAIS EM HEADLESS» no `xpz-llm-delegate/SKILL.md`. **Em aberto:** mitigação operacional padrão do painel (passar `-Agent <reviewer>` com `permission: bash: deny`, ver frente abaixo); a entrada está atrelada à **versão do opencode** (re-verificar se o comportamento headless mudar).
- **Ligação:** o eixo (ii) compartilha a **classe de risco exfiltração-local** com a frente «agente reviewer» abaixo — lá o vetor é a tool `read` (leitura irrestrita), aqui é o `bash` auto-aprovado.
- **Incidente real (2026-06-24, materialização do eixo `edit`):** numa pré-push reforçada com os 5 preferidos, o revisor opencode `kimi-k2.7-code` (agente default) **editou** `05b-procedure-relatorio-familias-e-templates.md` (corrigiu um typo) em vez de só reportar — **primeira vez** que o risco **escreveu** no repo (antes só medido como execução de `bash`). O orquestrador inspecionou o diff e adotou a correção, mas reforça a urgência: enquanto o agente reviewer read-only não for provisionado, despachar opencode-revisor é **inseguro por default**; mitigação interina = **preferir revisor read-only de outra família**. Regra operacional adicionada ao `xpz-llm-delegate/SKILL.md` (seção do limite opencode).

**Origem:** investigação do truncamento das vozes coder (2026-06-21/22); achado colateral medido ao testar `--agent plan` e agente custom. Conhecimento de **dispatch/segurança de revisor** — vai para o repo.

## Agente reviewer do opencode sem execução/escrita (leitura ainda irrestrita) — least-privilege parcial

- **Importância** — **ALTA / tratar com urgência** (é a mitigação concreta da frente «Painel de revisão por pares concede execução de código…» acima). **Recorte de urgência:** a parte do `bash: deny` (que **já funciona** empiricamente) é a mitigação acionável **agora**; conter o eixo de **leitura** (`read` irrestrito) segue como **pesquisa**, sem bloquear o ganho imediato.
- **Maturidade** — pesquisa/ideia (viabilidade do mecanismo **confirmada**; contenção de leitura e provisionamento em aberto).
- **Confirmado empiricamente:** um agente custom do opencode com `permission: bash: deny` (+`edit: deny`), selecionado por `opencode run --agent <nome>`, **remove** a tool bash em headless (o modelo respondeu em texto, `reason=stop`, ~5k tokens de input a menos). É a forma real de despachar uma voz coder **read-only** (o `--agent plan` **não** serve — auto-aprova). Análogo a (não "espelha") Gemini (`--approval-mode plan`) e Claude Code (`PermissionMode=plan`): mesma intenção, mecanismo distinto.
- **Limite residual (eixo de leitura):** mesmo negando `bash`/`edit`, a tool `read` (default) lê **qualquer arquivo** da máquina (config com baseURL/chaves, `.env`) — eixo de leitura/exfiltração **distinto** do gate, **não** fechado por esta frente. Logo o nome honesto é "reviewer **sem execução/escrita**", não "menor privilégio" pleno. Conter o `read` é **pesquisa aberta** (sandbox de FS / allowlist de paths; o adapter opencode **não** tem `-Cd`, e mesmo cwd não restringe `read` por caminho absoluto — adicionar `-Cd` seria melhoria geral do adapter, **não** fecha o eixo de leitura).
- **Provisionamento (em aberto):** o caminho canônico de agent configs do opencode não foi fixado (probes em `agent/` e `agents/`); o `xpz-skills-setup` **não** cobre agent configs (o `agentsPath` que ele resolve é do **MCP do Cursor**). Preferido: **versionar `reviewer.md` como artefato no repo** (ex.: `xpz-llm-delegate/agents/reviewer.md`) + passo de instalação ao `agentsPath` efetivo do opencode (a confirmar) — alternativa: ampliar o escopo do `xpz-skills-setup` (contrato próprio).
- **Precedente concreto (2026-06-24):** numa revisão por pares da correção do registro de elementos legados, criou-se **ad-hoc** um agente read-only `reviewer-ro` no `opencode.jsonc` **machine-level** (`~/.config/opencode/opencode.jsonc`), via bloco `agent` com `"tools": { "write": false, "edit": false, "bash": false, "patch": false }` (desabilita a ferramenta, mais forte que `permission: deny`), e as 3 vozes `ollama-cloud` do painel responderam **read-only** com sucesso (sem `bash`/`edit`). Isso **confirma o mecanismo via `tools: false`** (alternativa ao `permission: bash: deny` do item acima) e deixa um **molde pronto** a versionar quando esta frente for implementada — considerar promover o `reviewer-ro` a artefato no repo + passo de instalação no `opencode.jsonc`/`agent/` efetivo. O eixo de **leitura** (`read` irrestrito) **segue aberto**. O agente ficou instalado nesta máquina como **provisão interina**.

**Origem:** reenquadramento da «variante read-only para vozes coder» (acima) após confirmar o mecanismo `permission: bash: deny`. Convergido por **revisão por pares** (v1→v5, 5 vozes / 3 famílias).
...
- **Maturidade** — pronta para implementar (a rota já existe). Direção: quando o subagente nativo (anthropic) estiver indisponível por 529, o painel pode despachar a voz anthropic via **`scripts/Invoke-ClaudeCode.ps1 -Model claude-opus-4-7`** (backend Claude Code da `xpz-llm-delegate`) — mesma família, voz forte, independente do spawn nativo. Formalizar em `15-revisao-por-pares.md` (composição/resiliência) e/ou `xpz-llm-delegate/SKILL.md` (mecanismo). Cuidado: `claude-opus-4-7` casa a chave de destino `anthropic/claude-opus-4-7` (gate por destino inalterado); é voz da **mesma família** do orquestrador, então não substitui diversidade externa — só recupera a voz anthropic do painel. Origem: pré-push reforçada do pacote `.ContainsKey`+429 (2026-06-21).
...
- **Contrato de saída ESTRUTURADO dos adapters (surfado na frente A, `Invoke-LlmDelegatePanelDispatch`, 2026-06-22):** hoje os adapters sinalizam falha **lançando** `BLOCK:` em prosa pt-BR, sem status tipado (só o opencode tem `Get-OpenCodeCompletionVerdict`, e mesmo ele lança a truncagem como string). Um consumidor que precise classificar o resultado (o harness de despacho do painel) fica entre (a) classificar só por **sinais estruturais** (exit code + stdout vazio/não-vazio) e guardar a prosa **crua** no ledger — o que a frente A adotou (opção iii, por mérito) — ou (b) **parsear a prosa** (frágil). A frente futura: dar aos 6 adapters um **status de saída tipado** (ex.: verdict `ok|empty|timeout|unavailable|error|truncated` em campo machine-readable), que o harness e outros consumidores aproveitariam, eliminando classificação por prosa. **Decisão da frente A (revisão por pares v1→…):** NÃO refatorar os 6 adapters agora — escopo/risco desproporcional ao ganho imediato (o harness **não consome** o subtipo de falha; suas decisões load-bearing — "veio resposta?" e "é ollama-cloud p/ single-flight?" — são estruturais). Registrado aqui como frente própria.
...
- **Importância** — baixa-média (atrito real recorrente, com workaround operacional). Hoje o `preferred-reviewers.json` é **lista plana** (`Set-LlmDelegatePreferredReviewers.ps1` grava `{backend, targetModelKey, invokeArgs}`; `Resolve-LlmDelegatePreferredReviewers.ps1` devolve a composição sem ordem nem tier). A régua (`15-revisao-por-pares.md` / `xpz-llm-delegate`) manda **despachar a lista preferida inteira**, então adicionar revisores de reserva à lista plana os tornaria **co-iguais** (despachados sempre) — o oposto de "backup". Não há como registrar um revisor que só entre **quando o titular está indisponível**. Atrito concreto: as vozes `ollama-cloud/*` batem **cota semanal (HTTP 429)** cronicamente nesta máquina; sem tier, o orquestrador escolhe o fallback **manualmente em runtime** a cada painel.
- **Maturidade** — ideia (direção técnica clara; decisões de design e implementação em aberto).

**Direção:** estender o schema com um eixo de **tier** (ex.: `role: primary|backup`, ou `backupFor: <targetModelKey>`) e fazer o `Resolve-LlmDelegatePreferredReviewers.ps1` sugerir o backup **só quando** o titular correspondente está indisponível/falhando. Preservar o invariante **preferência ≠ autorização** (o gate `Resolve-LlmDelegateAuthorization.ps1` segue soberano por destino); a régua "despachar a lista inteira" passa a significar "a lista inteira de **titulares** + os backups dos que falharam". Self-test próprio; doc na seção de `preferred-reviewers` do `xpz-llm-delegate/SKILL.md` + paridade.

**Decisões em aberto:** (a) o que conta como "indisponível" que ativa o backup — cota/429, timeout, error, `noResponse`? (b) backup é **por-titular** (`backupFor`) ou **pool de reserva por família**? (c) como o `Resolve` sinaliza que o backup foi ativado; (d) interação com o piso de diversidade (`Resolve-LlmDelegatePanelDiversity.ps1`) e com o closeout (`Resolve-LlmDelegatePeerReviewCloseout.ps1`).

**Caso concreto que motiva (2026-06-28):** o usuário quer as 4 vozes **NVIDIA** (`nvidia/deepseek-ai/deepseek-v4-pro`, `nvidia/z-ai/glm-5.1`, `nvidia/moonshotai/kimi-k2.6`, `nvidia/minimaxai/minimax-m2.7`) como **backup** das 3 `ollama-cloud` (`deepseek-v4-pro`, `kimi-k2.7-code`, `glm-5.2`), usadas quando o ollama-cloud está em cota. Observado na mesma frente: as NVIDIA de raciocínio (`deepseek-v4-pro`/`glm-5.1`) tendem a **estourar 280s** (agravado por sessão concorrente) e as coder (`kimi-k2.6`/`minimax-m2.7`) **truncam por `tool-calls`** — então o backup precisa de **tolerância a falha por voz**, não só troca de provider. Por ora o fallback é **operacional** (orquestrador decide em runtime).
...
**Status (2026-07-01): frente documental COMPLETA.** Fase 0 empírica fechada; A1 (`api.md` elevado), A2 (`01e` tríade), B1 (`api-gam-runtime.md`) gravados no commit `7dc1bd3`. Nesta sessão: **A3 gravado** — recalibração do `03` distinguindo `API (cadeia)` de `API (from-spec)` em texto e nas 2 tabelas de risco (micro-notas de recalibração, `medio-contextual`, célula de `parent` corrigida, remissões aos satélites), passando por **revisão por pares** (6 versões, 3 famílias: anthropic nativo, openai/gpt-5.5, ollama-cloud deepseek/kimi/glm) que pegou e corrigiu um erro factual meu (afirmar que o from-spec não tinha `parent` nomeado — o molde `01e` tem `parent="PastaExemploApi"`). **A4 gravado** — `02` (Politica para API + dimensão GAM/runtime), `08` (`### API`), `10-base` (achado empírico preview≠import real); README avaliado = **sem edição** (índice de alto nível, sem conteúdo por tipo nem afirmação stale). **B gravado** — `xpz-builder/SKILL.md:52` expõe `api-gam-runtime.md` na enumeração de satélites (cadeia SKILL→`api.md`→`api-gam-runtime.md` amarrada; checklist já genérico). **Commitado em `main` (`d758f6c` + follow-up de paridade da pré-push), NÃO pushado.** Pré-push reforçada (13/14) executada (2 famílias: openai/Codex + anthropic nativo) — corrigidos 4 gaps de paridade (entrada CHANGELOG trilíngue, exceção "preview não valida API" no `xpz-msbuild-import-export/SKILL.md`, micro-notas de calibração nas afirmações stale "API caso único" em `01a`/`02`/`07`, e este bookkeeping). Pendente só: **push (autorização do usuário)**. Resíduos na KB de teste seguem (ver bullet acima).
