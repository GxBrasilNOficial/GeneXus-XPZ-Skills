...
- **Importância** — média (gap real com workaround manual; **escala para alta** assim que uma KB Java for aferida em produção). Hoje `deployment_hosting_kind` só aceita `dotnet-core-self-host`/`dotnet-framework-iis` (`Set-XpzKbSourceMetadataDeployment.ps1:93`) e o gate de deploy-bin é .NET por dentro (`web\bin\*.dll`+`*.config`+sentinela `GxNetCoreStartup.dll`). Uma KB gerada para Java/Tomcat não é representável nem verificável: o usuário contorna forjando metadata .NET, e o gate sairia procurando `.dll`/`GxNetCoreStartup.dll` numa app Java — falso-negativo de segurança (verde enganoso). Origem: proposta de usuária de outra máquina (KB Java/Tomcat), 2026-07-01.
