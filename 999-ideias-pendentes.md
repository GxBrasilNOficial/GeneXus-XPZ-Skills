...
- **Self-test do daemon VERMELHO (medido 2026-09-05):** `Test-ClaudeCodePreToolUseSafeAllowDaemonSelfTest.ps1` falha com **2 asserções** no cenário `F-stale-classify` — «baseline != allow» e «classify adversarial nao deferiu apos respawn (modulo novo nao importado?)». Achado por varredura da suíte inteira (130 self-tests; os outros 129 passam), **não** por mudança nesta área: nenhum arquivo do produto PreToolUse foi tocado desde então. Ainda **não diagnosticado** se é regressão real do respawn/importação de módulo, defasagem do próprio self-test, ou sensibilidade de ambiente (o cenário sobe daemon, mata e respawna, dependendo de pipe/mutex e timing). **Precisa fechar antes da Fase 4** — ligar `enforce` com o self-test adversarial vermelho seria ligar efeito real sem a rede de proteção que o justifica.
