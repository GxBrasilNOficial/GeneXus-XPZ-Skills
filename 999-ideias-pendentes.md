...
**Gap 1 — o dispatcher tem QUATRO pontos de registro, e o quarto não é citado em lugar nenhum.** Além de `$AdapterScript`, `$ExeParam` e `$ContentionKeys`, existe **`$AdapterDefaultTimeoutSec`** em `scripts/Invoke-LlmDelegatePanelDispatch.ps1` (todos os backends ativos têm entrada lá). Um backend registrado nos três primeiros e ausente no quarto **não** falha ruidosamente — apenas não tem timeout próprio. Nenhum documento ou checklist enumera os quatro.
...
**Direção sugerida (a decidir):** um check de pré-push que, ao detectar backend novo no `ValidateSet` de `-Backend`, exija presença nos **quatro** mapas do dispatcher; e uma nota normativa no `SKILL.md` de que `$ContentionKeys` se deriva do help da versão, nunca por cópia. Alternativa mais barata: só a nota normativa, sem mecanizar.
