...
- **Importância** — média (gap real com workaround). A camada de representação/governança do export legado GeneXus 9 já existe (registro `scripts/gx-legacy-export-element-registry.json` + doc-dono `01k-registro-elementos-legados.md` + 3 self-tests), mas o **motor** que a consome — parse do `ExportFile` GeneXus 9 (`<GXObject><Elemento>` por nome, sem GUID de tipo), materialização em `ObjetosDaKbEmXml` e integração no `Sync-GeneXusXpzToXml.ps1` — ainda não foi implementado. Sem ele, KBs GeneXus 9 não sincronizam pelo fluxo XPZ→XML; o `01k` declara explicitamente o motor como frente futura.
- **Maturidade** — ideia (decisões de design em aberto; o registro fixa a classificação `equivalent`/`orphan` e os `typeToken`, mas parser/materializador/integração no sync seguem a desenhar). Gatilho de caso real **atendido (2026-06-24)**: chegou um `ExportFile` GeneXus 9 real na máquina (amostra `FinGX90`, KB "Financeiro", gitignored em `Temp/`), que **confirmou tag/casing de 11 dos 12 elementos** e corrigiu o registro empiricamente (ver `CHANGELOG`); só `Theme` não foi observado. Resta desenhar/implementar o parser, o materializador e a integração no sync — o parser/§0.1 (XPath/estrutura GX9 exata) será re-revisado com a amostra na frente do motor.

Derivada da frente do registro v8 de elementos legados (ver `CHANGELOG`). É "o resto do PR de export legado": o registro/`01k` é a camada de representação; este motor é a execução.
