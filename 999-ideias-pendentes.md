...
- **Maturidade** — Fase 1 **pronta para implementar** (.NET-safe, não depende de KB Java); Fases 0/3 **pesquisa feita** (gatilho de caso real = environment Java em `wsEducacaoSpTeste`, montado por outra sessão). Design **congelado** após revisão por pares de 6 rodadas (v1→v6; 3 famílias; liberação unânime na rodada 6).
...
Fase 1 (.NET-safe, arranca já) não toca o motor de deploy-bin: registro + API + self-test de drift + `ArgumentCompleter`, com regressão .NET provada por golden tests. Fases 0/3 gated pela KB Java (Fase 3 = refactor de motor: aridade escalar→lista, fork por família `...CoreDotNet`/`...CoreJava`, `runtimeExclusionPrefixes` do catálogo). Congelamento vale para a **hipótese in-place**; topologia externa (`.war`/webapp fora de `web`) aciona o Plano B (Fase 3 replanejada). A implementação deve passar por nova revisão por pares.
