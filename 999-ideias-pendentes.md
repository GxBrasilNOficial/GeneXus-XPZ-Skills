...
- **Maturidade** — ideia (decisões de design em aberto; o registro fixa a classificação `equivalent`/`orphan` e os `typeToken`, mas parser/materializador/integração no sync seguem a desenhar). Gatilho de caso real: um `ExportFile` GeneXus 9 concreto na máquina (a amostra `GXW_alunoturma_re.xpz` do autor do PR de export legado não está disponível) — também o momento de confirmar tag/casing exatos de cada elemento, hoje os melhores conhecidos.
