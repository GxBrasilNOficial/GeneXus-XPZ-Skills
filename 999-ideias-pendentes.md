...
- **Maturidade** — Fase 1 **implementada** (2026-07-02): registro + API + `ArgumentCompleter` fail-soft + self-test de drift + golden de regressão .NET; convergida por revisão por pares da implementação (v1→v9, 3 famílias, zero gaps sobre a versão final) e pela pré-push reforçada. Fases 2/4/5 **pendentes** (fiação do metadata/`[ValidateSet]`, motor de gate por família, doc paridade `02`/`08`/README, testes gêmeos Java); Fases 0/3 **pesquisa feita**, gated pela KB Java (gatilho de caso real = environment Java em `wsEducacaoSpTeste`, montado por outra sessão). Design **congelado** após revisão por pares de 6 rodadas (v1→v6; 3 famílias; liberação unânime na rodada 6).
