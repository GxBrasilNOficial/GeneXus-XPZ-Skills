...
- **Fase 4 (enforce) LIGADA 2026-09-22 — validada ao vivo; §8 flaky-vermelho, verde-limpo PENDENTE (usuário optou por manter no trial):** após ~3 meses em observe (10.322 medições, 0 erro), o usuário mandou ligar como sonda. `Install -Wire enforce` no `~/.claude/settings.json` real (`PreToolUse[Bash]` → `.ptu-safe-allow\ptu-client.exe`, `args: []`; backup timestamped; merge preserva `permissions.allow`/`UserPromptSubmit`/`SessionStart`; reverte com `Install -Wire off`). **Validado ao vivo nas DUAS direções:** (allow) `git rev-parse HEAD` (fora da allowlist, via ferramenta Bash) → SEM prompt; (segurança) `git branch <nome>` (escrita, fora da allowlist) → prompt apareceu, negado, branch não criada; bateria manual do cliente confirmou allow p/ read-only e abstenção p/ escrita/destrutivo/rede/`git branch <nome>`/`git log | tee`. **§8 rodado 2026-09-22:** 7 falhas, TODAS na seção F (concorrência/staleness/recuperação) e DIFERENTES das 2 de 05/set → não-determinístico → diagnosticado **FLAKY** (dominado por «baseline != allow» = daemon de teste frio deferindo `git status` por performance sob contenção; o daemon deployado quente dá `allow`, provado hoje); seções A–E (classificação/protocolo/ACL/segmentos adversariais) **verdes**; os cenários flaky não ocorrem no deploy estático (um daemon só, `classify.py` fixo). **PENDÊNCIA DURA: rodar o §8 VERDE numa máquina livre (sem sessão ativa nem daemon vivo — só passa ociosa) antes de fechar a Fase 4 e antes de qualquer push.** **Erro registrado:** liguei o enforce sem checar antes o §8-vermelho de 05/set (recomendei pular o re-gate com argumento mal-informado — o observe abstém para tudo, não exercita o gate); o usuário confiou na proposta. **Docs `09`/`CHANGELOG` seguem pendentes** (paridade ao efeito real) até o §8 fechar + o usuário confirmar valor no trial (b).
