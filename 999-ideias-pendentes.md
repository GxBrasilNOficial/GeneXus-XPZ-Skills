...
- **Importância** — média (gap real com workaround manual; **escala para alta** assim que uma KB Java for aferida em produção). **Dor original (motivadora):** `deployment_hosting_kind` só aceitava `dotnet-core-self-host`/`dotnet-framework-iis` e o gate de deploy-bin é .NET por dentro (`web\bin\*.dll`+`*.config`+sentinela `GxNetCoreStartup.dll`); uma KB Java/Tomcat não era representável (o usuário contornava forjando metadata .NET) nem verificável (o gate sairia procurando `.dll`/`GxNetCoreStartup.dll` numa app Java — falso-negativo de segurança, verde enganoso). **Pós-Fase 2:** `java-tomcat` agora é **representável** (aceito pelo registro via `Set-XpzKbSourceMetadataDeployment.ps1`, não mais `[ValidateSet]` .NET-only) e o falso-negativo virou **skip explícito** (`skipped-hosting-unsupported`/exit 0 — skip ≠ deploy validado). **Falta a verificação real por família** (motor Java de deploy-bin) = Fase 3, gated pela KB Java. Origem: proposta de usuária de outra máquina (KB Java/Tomcat), 2026-07-01.
- **Maturidade** — Fase 1 **implementada** (2026-07-02): registro + API + `ArgumentCompleter` fail-soft + self-test de drift + golden de regressão .NET; convergida por revisão por pares da implementação (v1→v9, 3 famílias, zero gaps sobre a versão final) e pela pré-push reforçada. Fase 2 (fiação do registro nos consumidores via API pública + guardas de família dos Eixos C/B) e Fase 4 (doc paridade `02`/`08`/`09`/`10`/README/SKILLs) **implementadas** (2026-07-02, commit local `035f2c7` na `main`, **não pushado** — pendente de pré-push reforçada + ok de push); **Fase 0 (aterramento empírico) concluída** (2026-07-04) com um gap declarado (Q4/Q5 — teste controlado de dois builds não coletado) — ver [`evidence-catalog-java-tomcat.md`](evidence-catalog-java-tomcat.md); **Plano B acionado** (topologia Java/Tomcat = webapp desdobrado **externo** à árvore de output, aferido na KB `EBTECH` GX18U14, sabor Jakarta, na máquina de uma colega — a máquina de dev não tem licença Java). Fases 3 e 5 seguem **gated pela KB Java** (Fase 3 = motor de gate por família, **agora a replanejar sobre topologia externa**; Fase 5 = testes gêmeos Java). Design **congelado** após revisão por pares de 6 rodadas (v1→v6; 3 famílias; liberação unânime na rodada 6); a Fase 0 confirmou a ramificação Plano B já prevista no congelamento.
