...
**Decisões já fechadas pelo schema v2:** ativa em `quota`/`timeout`/`error`/`unavailable`/`noResponse`; fallback é **por-titular** (`fallbackChain[]`); ativação e skips aparecem no dispatcher/recibo; diversidade e closeout tratam `countsForDiversity=false` e `insufficientDiversityAfterFallback`. **Resíduos em aberto:** política de escopo do painel (`scopeDecision`/`defaultScope`) e single-flight fora da `fallbackChain`.
