...
## Descoberta do switch `-BlockCrossFlowDataSource` e da colisão cross-fluxo nas seções do `xpz-sync/SKILL.md`

- **Importância** — baixa (descoberta/conveniência; não-gap, levantado na revisão pré-push reforçada da frente cross-fluxo). O switch `-BlockCrossFlowDataSource` e os campos `CrossFlowCollisions`/`Writes[].CrossFlowCollision` já estão documentados no **workflow** (seção de interpretação de resultado do `xpz-sync/SKILL.md`) e nas bases (`02`/`08`/`01k`/`09`/`CHANGELOG`), mas duas seções do próprio `xpz-sync/SKILL.md` não foram tocadas: (a) a seção que **enumera os parâmetros** dos wrappers de atualização não lista `-BlockCrossFlowDataSource` (é opt-in); (b) a seção **«Exports legados (GeneXus 9)»** descreve o envelope legado (`dataSource="gx-legacy-export"`/`<GxLegacyPayload>`/`guid=""`) mas não alerta que, se o mesmo `FolderType|NormalizedName` já existir como **moderno** no acervo, a detecção cross-fluxo entra em ação. Reduz a descoberta do recurso por um agente que consulte só essas seções; não quebra contrato nem funcionalidade (o workflow cobre genericamente).
- **Maturidade** — pronta para implementar (dois acréscimos de ~2 linhas no `xpz-sync/SKILL.md`, sem mudança de código). Origem: pontos de «atenção documental» (explicitamente **não-gap**) do revisor `ollama-cloud/kimi-k2.7-code` na revisão pré-push reforçada da frente «detecção de divergência de origem (`dataSource`) cross-fluxo moderno↔legado» (2026-06-26), já implementada e pushada (`origin/main` em `07bd5a3`; ver `historico/IdeiasImplementadas_202606.md`).
