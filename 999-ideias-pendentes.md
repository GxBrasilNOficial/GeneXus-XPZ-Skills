...
- <!-- backend-parity: ignore --> `09-inventario-e-rastreabilidade-publica.md` — «nos **7** adapters Codex/…/Antigravity» → «nos adapters Codex/…/Antigravity» (a lista completa já está na própria frase; o número era redundante).
...
**Nível 3 — gate consultivo derivando do CÓDIGO (RESOLVIDO em 2026-08-19).** Implementado via `scripts/Test-PrePushBackendEnumerationParity.ps1` (+ self-test `scripts/Test-PrePushBackendEnumerationParitySelfTest.ps1` com 9 cenários e exit 0) derivando a verdade do `$AdapterScript` de `scripts/Invoke-LlmDelegatePanelDispatch.ps1`, e integrado no orquestrador mecânico `scripts/Invoke-PrePushMechanicalChecks.ps1`. Sinaliza unidades lógicas que enumerem subconjuntos próprios dos backends sem justificação/marcador de ignore (`<!-- backend-parity: ignore -->`).

**Invariante irmão absorvido no desenho do nível 3 (Gap 1 e Gap 2 resolvidos).** O dispatcher foi saneado promovendo `$AdapterCdCapable` para o topo junto aos demais mapas (`$AdapterScript`, `$ExeParam`, `$ContentionKeys`, `$AdapterDefaultTimeoutSec`), o self-test `Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1` ganhou AST Guard estrito de paridade entre os 5 mapas, `ValidateSet` de autorização e preferidos, e probe dinâmico de timeout; e a governança de `$ContentionKeys` foi documentada em `xpz-llm-delegate/SKILL.md`.
...
- **Importância** — média (pela escala: gap real com workaround manual — clicar o prompt de autorização; sem risco de dano). Prioridade de frente elevada pelo mantenedor (recalibrada em 2026-06-27 e reafirmada em 2026-09-22): o Claude Code é a ferramenta que ele mais usa e recomenda a quem usa as skills XPZ, então reduzir prompts de autorização para comandos read-only compostos que a allowlist literal não expressa beneficia esses usuários, não só o conforto local. O **cérebro viaja** (lógica versionada em `scripts/`), mas o **fio não** (a entrada de hook em `~/.claude/settings.json` é máquina-local).
- **Maturidade** — ideia (quanto ao que resta aberto: ampliar a gramática Bash ou Fase 5 PowerShell, com decisões em aberto; o núcleo Bash está implementado e em teste de uso, com a Fase 4 `enforce` ligada em 2026-09-22). Design **congelado** (v4 do daemon + plano v1) após revisão por pares (3 famílias) + Fase 0. **Passos A–G implementados:** fonte única Bash + self-test adversarial, **daemon `pwsh` persistente** (singleton/pipe/decisão), **cliente NativeAOT** (dispara-e-sai §5(b)), **gate de segurança §8**, e `Install-ClaudeCodePreToolUseSafeAllow.ps1` (deploy + wire `-Wire observe|enforce|off`). A saída §3.1 foi corrigida (**abster = não emitir `permissionDecision`**, verificado no fio real 2026-07-01). A Fase 3 (observe do FIO REAL, 2026-07-01 a 2026-09-22) fechou a latência de concorrência a favor, e a Fase 4 (`enforce`) foi ligada em 2026-09-22. **Pendente:** decisão do teste de uso (b); Fase 5 (PowerShell); G3 (distribuição via `xpz-skills-setup`); opcionais: endurecer o gate §8 contra daemon em segundo plano e auto-faxina de logs. Ver [`claude-code-pretooluse-implementacao-v1-plan.md`](claude-code-pretooluse-implementacao-v1-plan.md) e [`claude-code-pretooluse-daemon-design.md`](claude-code-pretooluse-daemon-design.md); trajetória, decisões e lições do host em [`historico/pretooluse-auto-allow-trajetoria-20260622-20260922.md`](historico/pretooluse-auto-allow-trajetoria-20260622-20260922.md).
- **[SUPERADO — o daemon implementado resolveu; fio quente p50 2,4 ms na Fase 3.]** **BLOQUEIO de latência (medido 2026-06-22):** ~520 ms/comando (caminho comum, só `pwsh`) e ~570 ms (candidato, sobe `python`) — dominado pelo **startup do `pwsh` por chamada**, **5× o orçamento p95 ≤ 100ms**. O enforce inline é **inviável**; o **daemon** deixou de ser dissidência e virou **pré-requisito da Fase 4**. Próxima frente real = daemon (processo persistente consultado pelo hook), não enforce.
- **Polaridade negativa descartada** (ver `998-ideias-descartadas-e-porque.md`): este é o oposto útil — auto-aprova em vez de barrar.
- <!-- backend-parity: ignore --> **Paridade documental — para o push FINAL da feature, não os intermediários:** enquanto a feature está **parcial e não-viva** (sem enforce; o wire `observe` é **passivo** — sobe o daemon e mede, mas **sempre abstém**, sem efeito real sobre as decisões do Claude Code), os scripts versionados **não exigem** entrada no `09` nem no `CHANGELOG`. A revisão pré-push reforçada de 2026-06-22 (painel de 5 preferidos, 3 famílias; 3 de 4 vozes — Codex, deepseek, glm — classificaram como follow-up; 1 voz minoritária recomendou bloqueio) confirmou isto. Quando o **enforce** for ligado (a feature passar a ter **efeito real** — o daemon subir em modo `observe` **não** conta, por ser medição passiva), aí sim: inventariar os scripts no `09` e registrar no `CHANGELOG`. `README` trilíngue: **avaliado e dispensado** (infra interna Claude-Code-only, não regra operacional de tipo GeneXus). **Cumprida em 2026-09-22:** com o enforce ligado, ponteiros no `09` e entrada no `CHANGELOG` gravados no mesmo dia; o usuário decidiu não adiar até o fim do teste de uso.
...
- **Fase 3 (observe) — RESULTADO após 2 dias de uso real (medido 2026-07-03):** 1406 medições no `ptu-observe-<hash>.log`. **O P0 de concorrência FECHOU A FAVOR: ZERO outcomes `busy`** (o daemon single-threaded **nunca enfileirou** em uso real, mesmo com sessões paralelas). Latência do fio quente p50 **2,4 ms** / p95 **5,4** / máx 76,4 (bem dentro do deadline de 80 ms; melhor que o ~17-24 ms estimado no Passo F — daemon aquecido). **14% dos comandos Bash (201/1406) seriam auto-aprovados** pelo enforce. Estabilidade: 0,3% cold + 0,4% deadline, **0 erros** de parse/identidade. Singleton **verificado OK** — os `loserSpawn` transitórios de cold-path (quando um cliente pega a janela de pipe-ausente) perdem o mutex e **saem sozinhos** (`exit 0`); raro (4 cold em 1406). **Conclusão: Fase 4 (enforce) desbloqueada no mérito** (o P0 que a condicionava não existe em uso real); falta a decisão datada + ligar o wire enforce + inventariar no `09`/`CHANGELOG` (paridade documental que o item «Paridade documental» desta entrada difere para o enforce/efeito real). (feito em 2026-09-22 — ver os itens seguintes).
- **[RESOLVIDO 2026-09-22 — os vermelhos eram contenção, não bug; ver o item «Fase 4 (enforce) LIGADA».]** **Self-test do daemon VERMELHO (medido 2026-09-05):** `Test-ClaudeCodePreToolUseSafeAllowDaemonSelfTest.ps1` falha com **2 asserções** no cenário `F-stale-classify` — «baseline != allow» e «classify adversarial nao deferiu apos respawn (modulo novo nao importado?)». Achado por varredura da suíte inteira (130 self-tests; os outros 129 passam), **não** por mudança nesta área: nenhum arquivo do produto PreToolUse foi tocado desde então. Ainda **não diagnosticado** se é regressão real do respawn/importação de módulo, defasagem do próprio self-test, ou sensibilidade de ambiente (o cenário sobe daemon, mata e respawna, dependendo de pipe/mutex e timing). **Precisa fechar antes da Fase 4** — ligar `enforce` com o self-test adversarial vermelho seria ligar efeito real sem a rede de proteção que o justifica.
...
- **Fase 4 (enforce) LIGADA 2026-09-22 — validada ao vivo; §8 VERDE quando rodado ISOLADO; enforce mantido (trial):** após ~3 meses em observe (10.322 medições, 0 erro), o usuário mandou ligar como sonda. `Install -Wire enforce` no `~/.claude/settings.json` real (`PreToolUse[Bash]` → `.ptu-safe-allow\ptu-client.exe`, `args: []`; backup timestamped; merge preserva `permissions.allow`/`UserPromptSubmit`/`SessionStart`; reverte com `Install -Wire off`). **Validado ao vivo nas DUAS direções:** (allow) `git rev-parse HEAD` (fora da allowlist, via ferramenta Bash) → SEM prompt; (segurança) `git branch <nome>` (escrita, fora da allowlist) → prompt apareceu, negado, branch não criada; bateria manual do cliente confirmou allow p/ read-only e abstenção p/ escrita/destrutivo/rede/`git branch <nome>`/`git log | tee`. **§8 (gate adversarial) PASSA VERDE quando rodado ISOLADO** (todos os processos ptu mortos + zero atividade Bash de qualquer sessão Claude Code durante; terminal puro) — confirmado 2026-09-22 (`OK: Test-ClaudeCodePreToolUseSafeAllowDaemonSelfTest.ps1`, 130 asserções). As 4 rodadas "vermelhas" anteriores (2→7→3→6 falhas, só seção F, não-determinísticas) eram **CONTENÇÃO, não bug**: com enforce vivo, **todo comando Bash de qualquer sessão Claude Code sobe um daemon concorrente**, e runs adjacentes se contaminam (achado do usuário: "parece que vc rodou junto comigo"). Seções A–E (classificação/protocolo/ACL/segmentos adversariais) verdes sempre. **A régua «§8 verde antes do enforce» ESTÁ cumprida** (com a ressalva de rodar isolado). Endurecer o gate para tolerar daemon de fundo = **nice-to-have, NÃO blocker**. **Erro registrado:** liguei o enforce sem checar antes o §8-vermelho de 05/set (recomendei pular o re-gate com argumento mal-informado — o observe abstém para tudo, não exercita o gate); o usuário confiou na proposta. Paridade `09`/`CHANGELOG` **feita em 2026-09-22** (sem adiamento, por decisão do usuário); o teste de uso (b) segue aberto para decidir o próximo passo (ampliar a gramática Bash ou Fase 5 PowerShell).
- **Achados do teste de uso (2026-09-22):** (a) parte do atrito vem do próprio agente: verificações compostas com `grep`/`sed`/`od`/`cut`, `$'...'` ou `cd` ficam fora da gramática (o hook se abstém) e ainda disparam checagens próprias do Claude Code (`ansi_c_string`, `cd`+git), independentes da allowlist; preferir as ferramentas nativas `Grep`/`Read`/`Glob`. (b) Candidata a frente: **ampliar a gramática Bash** com verbos só-leitura de alto uso (ex.: `grep`, `sed -n`), cada verbo com análise adversarial (`sed -i`/`w`, `grep -f`/`--pre`); pode render mais que a Fase 5. (c) **Em aberto:** não verificado se um `allow` do hook passa por cima dessas checagens próprias do Claude Code.
...
- <!-- backend-parity: ignore --> **Relacionada:** a entrada seguinte (migração stdin de `Invoke-Gemini`/`Invoke-Copilot`/`Invoke-Antigravity` + limite de 32KB) segue aberta, dependente de assinatura/acesso funcional para validação empírica.
...
- <!-- backend-parity: ignore --> **Importância** — média. As fatias **opencode síncrono + jobs com watcher** (ver `historico/IdeiasImplementadas_202608.md`), **Claude Code** (2026-09-01), **Codex** (`Get-CodexExecErrorMessage` com JSON nativo e sentinelas de cota/rate-limit/auth/serviço + `Resolve-CodexJobStatus` e `Invoke-Codex.ps1`, 2026-09-01; ver `historico/IdeiasImplementadas_202609.md`) e **Antigravity `public-review`** (`Resolve-AntigravityPublicReviewFailureReason` + recibo `Reason=quota`/`unauthenticated`, 2026-09-16; ver `historico/IdeiasImplementadas_202609.md`) já estão implementadas. Falta o mesmo diagnóstico nos **outros backends**: `Invoke-Gemini` e `Invoke-Copilot`.
...
- <!-- backend-parity: ignore --> **Maturidade** — **parcial**: fatia Antigravity feita com evidência real (`Individual quota reached`, 2026-09-16). Gemini/Copilot **não têm assinatura nesta máquina** para reproduzir o caso real — permanecem pendentes. **Não** fechar o item `999:857` (contrato tipado amplo dos adapters restantes).

<!-- backend-parity: ignore -->
**Origem:** frente da detecção de 429 no `Invoke-OpenCode` (2026-06-21), nascida de uma pré-push reforçada em que 3 modelos `ollama-cloud` (kimi/minimax/glm) "estouraram timeout" sem causa visível — era a **cota semanal** esgotada da conta ollama-cloud. Fatias opencode implementada em 2026-08-30, Claude Code e Codex implementadas em 2026-09-01, Antigravity `public-review` em 2026-09-16.
...
## Registro de backend novo na `xpz-llm-delegate` — pontos incompletos e sem gate [RESOLVIDO em 2026-08-19 — Gap 1 e Gap 2]

- **Importância** — média (não quebrava nada; resolvida a integridade com paridade de mapas e governança).
- **Maturidade** — **resolvida** em 2026-08-19.
...
**Casa da mecanização do Gap 1:** a entrada «Centralizar a fonte de verdade do conjunto de backends/adapters (três níveis)» — **nível 3** — implementou o gate `Test-PrePushBackendEnumerationParity.ps1` e absorveu o invariante de integridade estrutural no self-test do dispatcher.

**Direção do Gap 2:** nota normativa no `SKILL.md` de que `$ContentionKeys` se deriva do `--help` da versão instalada.
...
- **Modo adjacente — fabricação de contagem por lacuna no dossiê (medido 2026-08-24):** distinto do parecer raquítico acima (lá o revisor diz **pouco**; aqui ele diz número **errado** com confiança). Na rodada 1 da pré-push reforçada, o `minimax-m3` afirmou que a frente «reduziu o universo de 361 para 8 candidatas, eliminando 353» — números que não correspondiam a medição alguma, e com a causalidade invertida (361 era o total **pós**-mudança). Causa provável: o dossiê mostrava 33 candidatas e o `999` citava 361, sem o `truncatedProseCount=328` que reconcilia os dois — o campo existia no motor e era descartado pelo orquestrador. **Depois** de o campo passar a ser projetado (commit `9c508d8`), o mesmo modelo, no mesmo painel, citou as contagens corretas em duas rodadas seguidas e sem inventar aritmética. Sugere que parte da fabricação numérica é **induzida por lacuna de dado verificável no material**, não só propensão do modelo — e que suprir o dado é mitigação mais barata que qualquer heurística de piso. Não generalizar de um caso: vale como hipótese com uma observação a favor, não como lei.
...
- **Importância** — baixa-média (atrito real: bloqueou a 4ª voz do painel por ~15 min). O painel de revisão por pares pode usar o **subagente nativo** (ferramenta Agent/`Task`) com `targetModelKey` próprio (schema 3, `backend=orchestrator-native`) — a família no piso é o Criador dessa chave (tese B), **não** a ferramenta orquestradora nem um modelo “fixado na sessão”. Quando a API do criador escolhido está sobrecarregada, o spawn pode falhar com **HTTP 529 Overloaded** repetido (observado 2026-06-21: `opus-4.8` deu 529 ×4 seguidos), e **não há fallback automático** nesta frente.
- **Maturidade** — pronta para implementar (a rota CLI já existe). Direção: quando o nativo com criador `anthropic` estiver indisponível por 529, o painel pode despachar a voz anthropic por um revisor `backend=claude-code` com `model=claude-opus-4-7`; no painel, essa rota deve usar o contrato assíncrono `scripts/Invoke-ClaudeCodeAsync.ps1` via dispatcher, não o adapter síncrono. É recuperação do **criador `anthropic`** por CLI, independente do spawn nativo — não confundir com “voz da ferramenta corrente”. Formalizar em `15-revisao-por-pares.md` (composição/resiliência) e/ou `xpz-llm-delegate/SKILL.md` (mecanismo). Cuidado: `claude-opus-4-7` casa a chave de destino `anthropic/claude-opus-4-7` (gate por destino inalterado); só recupera a voz anthropic do painel, não substitui diversidade de outros criadores. **Não** implementar o fallback 529 nesta frente de schema 3/cascata. Origem: pré-push reforçada do pacote `.ContainsKey`+429 (2026-06-21); alinhada ao contrato assíncrono do Claude Code no painel em 2026-07-29; premissa alinhada à tese B em 2026-08-27.
...
## Robustez do perfil Antigravity `public-review`: cinco riscos residuais

- **Importância** — média para o item (1), baixa-média para os demais. Nenhum bloqueia push; todos vieram de leitura de código, não de incidente observado.
- **Maturidade** — ideia; cada item pede desenho próprio antes de mexer.

1. **`Get-WorkspaceAncestorDiagnostics` sobe até a raiz do drive** (`scripts/Invoke-Antigravity.ps1`). Se **qualquer** ancestral de `%TEMP%` tiver `.git` — home versionado com dotfiles é prática comum —, o backend fica **permanentemente** em `unsafeWorkspace`, e a mensagem não aponta qual ancestral causou a recusa. Modo de falha silencioso e difícil de diagnosticar: o revisor some do painel sem causa legível. É o mais concreto dos cinco.
2. **Kill por PID observado.** Os PIDs dos descendentes são coletados durante a vida do runner; no `finally` o processo já saiu. Se o SO reciclou um PID nesse intervalo, `Stop-Process -Force` atinge processo de terceiros. Janela estreita, consequência alta — vale confrontar contra job object ou verificação de identidade antes do kill.
3. **Polling de `Get-CimInstance Win32_Process` a 10 Hz por até 300 s por revisor**, com o `deadline` avaliado **depois** da enumeração: em painel paralelo é carga não trivial, e o timeout escorrega pelo custo da própria consulta.
4. **`agy --version` virou dependência dura** do recibo sem estar no contrato de descoberta validado por `Test-AntigravityHelpSupportsContract`, que existe justamente para «distinguir o CLI certo». Os três ramos de erro passaram a ter cobertura de self-test, mas o contrato de descoberta não conhece a flag.
5. **Comentários anti-regressão removidos** de `AntigravityCliSupport.ps1` (o porquê de `exhausted` não entrar solto no padrão de cota, e de `\bauth\b` não casar «authentication»). O conhecimento sobrevive no `SKILL.md`, mas quem editar o regex não vê o aviso ao lado dele — que é onde ele protege.

**Origem:** 2ª rodada do painel da pré-push reforçada de 2026-08-24, revisor Claude Code (`anthropic/claude-opus-5`), na seção «riscos de robustez» do parecer. Os três **gaps** da mesma leitura (precedência de cota sobre `reason` do recibo, ramo morto `refusedSensitivity`, cobertura ausente do suporte) foram corrigidos na hora; estes cinco ficaram por decisão humana de não misturar robustez especulativa com correção de defeito confirmado.

## `Test-PrePushNewTokenPropagation`: par por morfema genérico de sufixo (`Path`) cruza contratos distintos

- <!-- backend-parity: ignore --> **Importância** — média (maior que a do falso-positivo de «introduzido» acima, e por mecanismo **diferente**). Os adapters citados a seguir são **amostra dos arquivos afetados**, não enumeração do conjunto de backends. `Test-TokensRelated` aceita o par quando prefixo **ou** sufixo comum tem >= 4 chars. `Path`, `Name`, `List` e afins são sufixos de **tipo**, não raiz de contrato: casam qualquer par de parâmetros que termine igual, ainda que pertençam a contratos sem relação. Medido na pré-push de 2026-08-24 (intervalo do perfil Antigravity `public-review`): dos 3 pares sobreviventes, `ReceiptPath`/`MessagePath` (123 candidatas) e `ScratchPath`/`MessagePath` (122) responderam por **245 das 361** — parâmetros de contratos distintos: os de origem pertencem só ao `Invoke-Antigravity.ps1`, o pareado é comum aos adapters (contrato em `xpz-llm-delegate/SKILL.md`, dono normativo). Dos 24 arquivos com candidata desse par, **13 nem mencionam Antigravity/`agy`** (`Invoke-OpenCode.ps1`, `Invoke-Codex.ps1`, `Invoke-Gemini.ps1`, `Start-ClaudeCodeJob.ps1` e outros): nenhum poderia estar defasado quanto a `-ScratchPath`.
- **Risco real** — não é custo de triagem: o teto de truncamento vale só para `prose`, e a disciplina de classe protege as não-prosa (ver `13-revisao-pre-push.md`, passo 2, dono normativo). É **falso negativo por diluição**: com `truncatedProseCount=328` na mesma medição, uma candidata de prosa legítima tem alta chance de cair entre as truncadas e nunca ser vista. O risco fica confinado à classe `prose` — a de menor valor acionável — justamente porque aquela disciplina já cobre as demais.
...
Sonda em `Domain:Aliquota`: `who-uses(Domain:Aliquota)` materializa relações **`based_on_domain`** (Property `idBasedOn`). Com o extrator **12**, a origem deixa de ser só `Attribute` — Procedure/SDT/WebPanel/Transaction/etc. no escopo também entram; `who-uses` conta **linhas de relação**. O salto 2 (cada atributo afetado → quem o consome) **continua** no buraco do caso destrutivo (consumo de atributo no corpo é cego). Receita = `who-uses(Domain)` para o conjunto de atributos/objetos baseados (índice) → por atributo, **grep + build** para consumidores de Attribute.
...
O índice **não é cego em bloco** — a cegueira é **estreita e específica**. Cobre bem: objeto→objeto (`calls_procedure`, `navigates_explicit_table`, `loads/saves_business_component`, `formula_*`, `workwith_*`), casas estruturais do atributo (`has_level_attribute`, `has_index_member_attribute`) e **`idBasedOn`→`Domain` (`based_on_domain`)** em Attribute **e** demais tipos do escopo do extrator 12. O **único** buraco material remanescente nesta família é **"objeto X lê/escreve atributo A no `Source`/conditions"** — relação faltante `references_attribute`, não carência ampla de Domain.
...
## Residuais pós-extrator-12 (`idBasedOn`→`Domain`) — adiados de propósito

- **Importância** — baixa / média (forma escapada: baixa no FabricaBrasil; unificação de módulo: média de manutenção; homônimos: baixa com limitação declarada).
- **Maturidade** — ideia (comandos/regex da forma escapada anexáveis; unificação toca `resolve_custom_type_target`/`exo:`).

1. **Forma escapada XML de `idBasedOn`** (ex.: entidades além de `&#xA;` já normalizado): medido 181/0 resolvíveis em FabricaBrasil e 28/0 em `Gx_wsEducacaoSpTeste` — **adiada**; não bloqueia o extrator 12.
...
- Cada `def extract_*` devolve `list[Evidence]` com `relation_kind` e é registrado; espelhar:
  ...
  - **idBasedOn**: `extract_idbasedon_domain_evidence` (`based_on_domain`, regras `attribute_idbasedon_domain` / `object_idbasedon_domain`) + regex `idBasedOn`. O value pode ser `Domain:X` **ou** `Attribute:X` (membro de SDT é `Attribute:OperacaoItemContaId`). **Domain:** já é coberto pelo extrator 12 (incl. sufixo de módulo com conferência de `fullyQualifiedName`). Valor `Attribute:` **não** gera `based_on_domain` — **estender** para emitir `references_attribute` quando o value for `Attribute:` — quase uma extensão, não código do zero.
...
- <!-- backend-parity: ignore --> **`added` vs `added`+`modified`**: Claude recomenda só `added` (tocar um dos ~17 legados ausentes geraria warn); Codex/minimax recomendam `added`+`modified` (tocar um legado ausente é o momento barato de decidir se entra). Tratar `A`/`M`/`R`(destino), nunca `D`.
