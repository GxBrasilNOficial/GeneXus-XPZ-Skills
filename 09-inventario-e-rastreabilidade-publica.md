...
- `scripts/Set-LlmDelegatePreferredReviewers.ps1` (motor) — persiste curadoria machine-level `preferred-reviewers.json` schema v2: titulares validados/ordenados por `rank` positivo/único, com `rank` omitido atribuído depois do maior `rank` explícito válido, `targetModelKey`, `backend`, `invokeArgs.backend`, `fallbackPolicy` restrito ao contrato suportado (`quota`/`timeout`/`error`/`unavailable`, normalizando legado com `noResponse`) e `fallbackChain[]` ordenado `0..N`; valida veto duro, backend, rank, política de fallback, divergência backend/invokeArgs, duplicidade/ciclo, `-Preview`, backup e escrita atômica. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-LlmDelegatePreferredReviewersSelfTest.ps1. Tokens: `OK: Test-LlmDelegatePreferredReviewersSelfTest.ps1`.
- `scripts/Resolve-LlmDelegatePreferredReviewers.ps1` (motor) — lê schema v1/v2, cruza curadoria com `capabilities.json` (`availableInManifest`/`capability`/`diagnostics`), devolve titulares ordenados por `rank` + `fallbackChain`; normaliza `fallbackPolicy` legado com `noResponse` para o contrato vigente; bloqueia hard veto, rank inválido/duplicado, `fallbackPolicy` fora do contrato suportado, ciclo/duplicidade e divergência `invokeArgs.backend`; preferência ≠ autorização. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-LlmDelegatePreferredReviewersSelfTest.ps1.
