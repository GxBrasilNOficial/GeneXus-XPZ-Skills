...
- `scripts/GeneXusKbDeploymentEnvironmentSupport.ps1` (support-library) — lê e valida os campos de environment/deploy de `kb-source-metadata.md`. **Fase 3 (co-gate Java):** também lê 3 mapas por-env dedicados ao alvo Java externo — `kb_environment_servlet_dirs` (`SERVLET_DIR`), `kb_environment_app_package` (`com\<kb>`) e `kb_environment_servlet_flavor` (`jakarta`|`javax`) — aditivo (não afeta .NET; ausentes = `[ordered]@{}` vazio). Dono: xpz-kb-parallel-setup/SKILL.md. Validação: via consumidores.
