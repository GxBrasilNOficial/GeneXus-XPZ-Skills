...
- `scripts/Start-OpenCodeJob.ps1` (motor) — disparo assíncrono não-bloqueante do opencode (stdin-based, prompt em `<GUID>.stdin.txt`; runner mínimo grava `<GUID>.exitcode.txt`; campo `result` inicial é caminho previsto, não aceite); **default `-Agent reviewer-ro` + pré-check fail-closed ANTES do spawn**. Dono: xpz-llm-delegate/SKILL.md. Validação: via Test-OpenCodeReviewerRoSelfTest.ps1, Test-LlmDelegateStdinHandlingSelfTest.ps1 e demais self-tests do backend.
- `scripts/Watch-OpenCodeJob.ps1` (motor) — monitor incremental do job opencode; grava `<GUID>.result.json` `schemaVersion=2` aceito/rejeitado, com `resultAccepted`, `acceptedFinalText`, `finalTextDisposition`, `rejectionReason`, `watcherExitCode`, `opencodeExitCode`, `hasStepFinish` e `completionVerdict`; fallback para `build`/default invalida aceite (`watcherExitCode=20`). Dono: xpz-llm-delegate/SKILL.md. Validação: via Test-OpenCodeStreamSupportSelfTest.ps1; a detecção de fallback (guard) por Test-OpenCodeReviewerRoSelfTest.ps1 (caso (e)).
