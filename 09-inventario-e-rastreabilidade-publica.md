...
- `scripts/gx-legacy-export-element-registry.json` (registro) — classifica elementos de export legado GeneXus 9 (`<GXObject><Elemento>` por nome, sem GUID de tipo) como `equivalent` (reusa tipo/`folderName` do catálogo via `catalogType`) ou `orphan` (`materializedFolderName` + `typeToken` `gxlegacy/<Elemento>` + `modernSuccessor`/`successorRelation`); fora do catálogo moderno por design. Camada de representação/governança. O **parser/conversor** legado (`scripts/GeneXusLegacyExportFileSupport.ps1`, ver abaixo) já consome este registro; a **integração no fluxo de sync/inventário** (materialização no acervo) é a frente em andamento. Dono: 01k-registro-elementos-legados.md. Validação: Test-GeneXusLegacyRegistrySchemaSelfTest.ps1, Test-GeneXusLegacyRegistryDisjointSelfTest.ps1.
...
- `scripts/GeneXusFileBaseNameNormalizationSupport.ps1` (support-library) — fonte única de `Normalize-FileBaseName` (substitui caractere inválido de nome de arquivo por `_`, remove ponto final; preserva ponto interno); dot-sourced por `Sync-GeneXusXpzToXml.ps1` e (frente futura, dependente de amostra GX9) pelo conversor de export legado, garantindo detecção de colisão `FolderType|NormalizedName` idêntica entre os caminhos. Dono: cabeçalho do script. Validação: Test-GeneXusFileBaseNameNormalizationSelfTest.ps1. Tokens: `OK: Test-GeneXusFileBaseNameNormalizationSelfTest.ps1`.
