...
- `scripts/AntigravityCliSupport.ps1` (support-library) — resolve `agy.exe` do PATH ou `%LOCALAPPDATA%\agy\bin\agy.exe`, valida contrato de flags (`--print`/`--prompt` e `--mode`), extrai mensagens de erro (formas flexionadas `authenticat*`/`authoriz*`/`sign in` — `\bauth\b` isolado não casa «authentication»; `credential` fica fora por ser linha informativa de execução normal) e exporta regex `$quotaFailurePattern` para a suíte de autoteste e utilitários. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-AntigravityCliSupportSelfTest.ps1. Tokens: `OK: Test-AntigravityCliSupportSelfTest.ps1`.
