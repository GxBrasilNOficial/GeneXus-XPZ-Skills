...
- `scripts/Build-PrePushReviewDossier.ps1` (motor) — monta o dossiê do modo assistido por dossiê da pré-push reforçada (revisor semantic-only): Seção A (git bruto) + Seção B (container do `-AsJson` do orquestrador mecânico, rotulado «candidatas, não verdade») + sentinelas `HEAD:`/`END_DOSSIER sha256=`; entregue via stdin ao revisor sem shell. Dono: 13-revisao-pre-push.md (modo); política de painel: 14. Validação: Test-BuildPrePushReviewDossierSelfTest.ps1.
