...
- `scripts/Test-GeneXusRuntimeFreshness.ps1` (motor) — diagnóstico **somente-leitura** de frescor de runtime (Eixo C): lê `nav_objs.xml` (`genreq`/`nogenreq`) + timestamps dos artefatos gerados vs `-ImportedAt`; saída `runtime-fresh`/`runtime-stale`/`runtime-unknown`. **Paridade Java/Tomcat (guarda de família do Eixo C, desde a Fase 2):** aceita `-DeploymentHostingKind` **opcional**; família não-.NET (`java-tomcat`, Eixo C `runtimeSupportState=recognized-no-engine`, Pós-v1 — o Eixo A roda o co-gate Java) → `status=skipped-hosting-unsupported` (exit 0) **sem** derivar `CSharpModel\web`; ausente/vazio → comportamento .NET de hoje (zero regressão); valor fora do registro → `runtime-hosting-kind-invalido` (exit não-zero). Dono: xpz-msbuild-build/SKILL.md; também xpz-msbuild-import-export/SKILL.md, xpz-kb-parallel-setup/SKILL.md. Validação: Test-GeneXusRuntimeFreshnessHostingGuardSelfTest.ps1. Tokens: `GENEXUS_RUNTIME_FRESHNESS_HOSTING_GUARD_SELFTEST_OK`.
