...
- `scripts/Resolve-LlmDelegatePeerReviewCloseout.ps1` (motor) — verifica fechamento da revisão por pares; bloqueia oferta de curadoria ausente, estado auditável ausente, estados incompletos, `notAttempted` primário, skip contando diversidade, `insufficientDiversityAfterFallback` e vN+1 pendente/declínio não auditado; ecoa `attemptRole`/`fallbackOf`/`countsForDiversity`/`vNextState`; stateless, não grava preferência, não decide autorização e não recalcula diversidade. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-LlmDelegatePeerReviewCloseoutSelfTest.ps1. Tokens: `OK: Test-LlmDelegatePeerReviewCloseoutSelfTest.ps1`.
