...
- `scripts/Test-GeneXusFrontAcervoDrift.ps1` (motor) — gate 9-FD drift frente-vs-acervo por `lastUpdate` (`front-older-than-acervo`=fail bloqueia, equals/unparseable=warn, newer/new-object=info) e por `Object/@type` divergente quando frente e acervo compartilham o mesmo `guid` (`front-object-type-drift`=fail, `objectGuid` normalizado e `message`; acervo ambíguo por GUID = `front-object-type-drift-ambiguous-acervo` info com `candidateAcervoPaths`, sem escolher tipo oficial arbitrário); opera sobre frente existente. Dono: xpz-builder/SKILL.md. Validação: Test-GeneXusFrontAcervoDriftSelfTest.ps1, via fluxo do empacotamento.
- `scripts/Copy-GeneXusAcervoToFront.ps1` (motor) — resolve drift temporal / seed da frente: copia acervo→frente com bump de `lastUpdate`, `-ObjectList`/`-ObjectNames`/`-ObjectGuids`, `-DryRun`; bloqueia autocópia quando há `Object/@type` divergente por mesmo `guid`; JSON no stdout. Dono: xpz-builder/SKILL.md; também 02-regras-operacionais-e-runtime.md. Validação: Test-CopyGeneXusAcervoToFrontSelfTest.ps1.
