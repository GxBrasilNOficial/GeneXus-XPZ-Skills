...
- `scripts/Start-OpenCodeJob.ps1` (motor) — disparo assíncrono não-bloqueante do opencode (stdin-based, sem runner, prompt em `<GUID>.stdin.txt`; espelha Start-CodexJob); **default `-Agent reviewer-ro` + pré-check fail-closed ANTES do `Start-Process`** (o spawn é a barreira; o assíncrono não tem finally-remove). Dono: xpz-llm-delegate/SKILL.md. Validação: via Test-OpenCodeReviewerRoSelfTest.ps1 e demais self-tests do backend.
- `scripts/Watch-OpenCodeJob.ps1` (motor) — monitor incremental do job opencode; **pós-check assíncrono = diagnóstico** (dot-sourceia `OpenCodeReviewerRoGuard.ps1` e usa `Test-OpenCodeReviewerRoFallbackWarning` → `fallbackToBuild` no `result.json` + aviso; não bloqueia — a barreira é o pré-check no spawn). Dono: xpz-llm-delegate/SKILL.md. Validação: via Test-OpenCodeStreamSupportSelfTest.ps1; a detecção de fallback (guard) por Test-OpenCodeReviewerRoSelfTest.ps1 (caso (e)).
...
- `scripts/OpenCodeStreamSupport.ps1` (support-library) — parsing do stream do opencode (`ConvertFrom-OpenCodeStreamLines`, `Get-OpenCodeFinalText`/`Get-OpenCodeAllText`, `Get-OpenCodeCompletionSignal`/`Get-OpenCodeCompletionVerdict` para detecção de truncamento por `reason`; resposta final = concatenação da última mensagem, erro do stream como BLOCK/`status:error`) e **diagnóstico de limite de uso do provider** (`Get-OpenCodeUsageLimitError`: lê o log do opencode e detecta HTTP 429 quando o adapter estoura por timeout; `-LogDir` para fixture); consumido por Invoke-OpenCode/Watch-OpenCodeJob. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-OpenCodeStreamSupportSelfTest.ps1, Test-OpenCodeUsageLimitDetectionSelfTest.ps1. Tokens: `OK: Test-OpenCodeStreamSupportSelfTest.ps1`, `OPENCODE_USAGE_LIMIT_DETECTION_SELFTEST_OK`.
- `scripts/OpenCodeReviewerRoGuard.ps1` (support-library) — guard least-privilege do agente `reviewer-ro` (dot-source), compartilhado por Invoke-OpenCode/Start-OpenCodeJob/Watch-OpenCodeJob: pré-check fail-closed (estático + `opencode agent list` allow-set resolvido EXATAMENTE `{read,grep,glob,list}` + versão testada, com motivo `static`/`version`/`allowset`/`agentlist`-transitório), `Test-OpenCodeAgentResolves` (opt-out), pós-check de fallback e leitor JSONC. Fixtures/claims em `xpz-llm-delegate/fixtures/opencode-reviewer-ro/`. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-OpenCodeReviewerRoSelfTest.ps1. Tokens: `OPENCODE_REVIEWER_RO_SELFTEST_OK`.
