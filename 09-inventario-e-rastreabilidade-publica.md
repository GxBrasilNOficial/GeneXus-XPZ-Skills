...
- `scripts/New-LlmDelegatePeerReviewArtifacts.ps1` (motor) — preparador transacional de artefatos da rodada: `-ManuscriptText` ou `-ManuscriptPath`, `ReviewersJson` com raiz array e `invokeArgs` objeto, staging `.prepare-*`, saída JSON única e nenhum dispatcher/adapter. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-NewLlmDelegatePeerReviewArtifactsSelfTest.ps1. Tokens: `OK: Test-NewLlmDelegatePeerReviewArtifactsSelfTest.ps1`.
