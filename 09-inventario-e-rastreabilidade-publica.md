...
- `scripts/Invoke-LlmDelegatePanelDispatch.ps1` (motor) — harness MECÂNICO de despacho+coleta: valida `invokeArgs.backend` antes do dispatcher, despacha titulares em paralelo, registra `attemptRole`/`rank`/`fallbackChain`/`countsForDiversity`, ativa `fallbackChain` ordenado para `quota`/`timeout`/`error`/`unavailable`/`noResponse`, mata processo filho de fallback recursivo ao exceder timeout, marca `skippedAfterSuccess`/`skippedByPolicy`/`notAttempted`, emite `panel-summary.json` + `manifest.json` + ledger; NÃO injeta nativo, NÃO calcula piso/closeout/triagem/convergência (do orquestrador). Params TEST-ONLY: `-OpenCodeConfigPath`/`-CodexConfigPath`/`-BackendExeMap`. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1. Tokens: `OK: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1`.
