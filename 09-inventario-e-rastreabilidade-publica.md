...
- `scripts/ClaudeCodeCliSupport.ps1` (support-library) — resolve `claude.exe` do PATH (versão mínima `2.1.118`), valida contrato de flags, **descarta ruído de ambiente do `stderr` antes de classificar** (avisos de `permissions.allow` descartadas, que aparecem inclusive em execução bem-sucedida), emite `max-turns-exhausted` para esgotamento de turno (texto `Reached max turns`, mensagem `Reached maximum number of turns (N)` de `errors[]` ou subtype `error_max_turns` do stream), detecta recusa por `workspace-not-trusted`, extrai a falha de evento do stream JSONL (`Get-ClaudeCodeStreamEventErrorText`, lendo `subtype`, `terminal_reason`, `result` e `errors[]` — no evento real de `error_max_turns` só `errors[]` traz a mensagem legível), preserva a evidência bruta com instrução de coleta segura e classifica o status do job — no assíncrono pelos **mesmos** detectores do síncrono. Exit codes do Claude Code são tratados só como sinal operacional documentado no dono normativo (`0` sucesso; `1` falha genérica quando sem causa textual; `2` bloqueio de hook, não contrato geral do `claude -p`; `137` processo morto/kill/OOM em contexto de instalação). Dono: xpz-llm-delegate/SKILL.md. Validação: Test-ClaudeCodeCliSupportSelfTest.ps1. Tokens: `OK: Test-ClaudeCodeCliSupportSelfTest.ps1`.
