...
- `scripts/Set-LlmDelegatePreferredReviewers.ps1` (motor) — persiste curadoria machine-level `preferred-reviewers.json` schema v2: titulares com `rank`, `targetModelKey`, `backend`, `invokeArgs.backend` e `fallbackChain[]` ordenado `0..N`; valida veto duro, backend, divergência backend/invokeArgs, duplicidade/ciclo, `-Preview`, backup e escrita atômica. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-LlmDelegatePreferredReviewersSelfTest.ps1. Tokens: `OK: Test-LlmDelegatePreferredReviewersSelfTest.ps1`.
- `scripts/Resolve-LlmDelegatePreferredReviewers.ps1` (motor) — lê schema v1/v2, cruza curadoria com `capabilities.json` (`availableInManifest`/`capability`/`diagnostics`), devolve titulares + `fallbackChain`; bloqueia hard veto, ciclo/duplicidade e divergência `invokeArgs.backend`; preferência ≠ autorização. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-LlmDelegatePreferredReviewersSelfTest.ps1.
...
- `scripts/Invoke-LlmDelegatePanelDispatch.ps1` (motor) — harness MECÂNICO de despacho+coleta: valida `invokeArgs.backend` antes do dispatcher, despacha titulares em paralelo, registra `attemptRole`/`rank`/`fallbackChain`/`countsForDiversity`, ativa `fallbackChain` ordenado para `quota`/`timeout`/`error`/`unavailable`/`noResponse`, marca `skippedAfterSuccess`/`skippedByPolicy`/`notAttempted`, emite `panel-summary.json` + `manifest.json` + ledger; NÃO injeta nativo, NÃO calcula piso/closeout/triagem/convergência (do orquestrador). Params TEST-ONLY: `-OpenCodeConfigPath`/`-CodexConfigPath`/`-BackendExeMap`. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1. Tokens: `OK: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1`.
