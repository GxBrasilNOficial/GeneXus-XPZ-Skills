...
- `scripts/gx-legacy-export-element-registry.json` (registro) — classifica elementos de export legado GeneXus 9 (`<GXObject><Elemento>` por nome, sem GUID de tipo) como `equivalent` (reusa tipo/`folderName` do catálogo via `catalogType`) ou `orphan` (`materializedFolderName` + `typeToken` `gxlegacy/<Elemento>` + `modernSuccessor`/`successorRelation`); fora do catálogo moderno por design. Camada de representação/governança — o motor de export/materialização (parse do `ExportFile` GX9, integração no sync) é frente futura, não incluída aqui. Dono: 01k-registro-elementos-legados.md. Validação: Test-GeneXusLegacyRegistrySchemaSelfTest.ps1, Test-GeneXusLegacyRegistryDisjointSelfTest.ps1.
