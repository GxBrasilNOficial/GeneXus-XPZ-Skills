...
- `scripts/Watch-OpenCodeJob.ps1` (motor) — monitor incremental do job opencode; **pós-check assíncrono = diagnóstico** (`fallbackToBuild` no `result.json` + aviso; não bloqueia — a barreira é o pré-check no spawn). Dono: xpz-llm-delegate/SKILL.md. Validação: via Test-OpenCodeStreamSupportSelfTest.ps1.
