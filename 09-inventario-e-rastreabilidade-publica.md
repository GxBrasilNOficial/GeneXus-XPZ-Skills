...
- `scripts/GeneXusLegacyExportFileSupport.ps1` (support-library) — adapta `ExportFile` legado (`GXObject`/`GXAtt`, ex.: GX9) para envelopes materializáveis (`Object`/`Attribute`) com `dataSource="gx-legacy-export"` e `GxLegacyPayload`; itens adaptados expõem `Guid=''` para não participar de rename por GUID em `-FullSnapshot`. Consumido por `Sync-GeneXusXpzToXml.ps1` e `Get-GeneXusImportPackageObjectInventory.ps1`. Dono: xpz-sync/SKILL.md; também xpz-reader/SKILL.md. Validação: Test-GeneXusLegacyExportFileSelfTest.ps1, Test-XpzSyncLegacyExportFullSnapshotSelfTest.ps1. Tokens: `Test-GeneXusLegacyExportFileSelfTest: OK`, `OK: Test-XpzSyncLegacyExportFullSnapshotSelfTest.ps1`.
