...
- `scripts/Test-GeneXusDeployBinDotNetGoldenSelfTest.ps1` (motor) — golden de regressão .NET da fachada `Test-GeneXusDeployBinFreshness.ps1` (caso **aceito** `dotnet-framework-iis` com DLL fresca → `fresh`; caso **rejeitado** `java-tomcat` → `skipped` pela fachada .NET-only ainda não fiada ao registro), com timestamps/paths normalizados (`<TS>`/`<ROOT>`); prova que a Fase 1 preserva o comportamento .NET (motor escalar intocado). Dono: xpz-msbuild-build/SKILL.md (família deploy-bin). Validação: self-test (`-UpdateBaseline` para recapturar conscientemente na Fase 2). Tokens: `GENEXUS_DEPLOY_BIN_DOTNET_GOLDEN_SELFTEST_OK`.
