...
- `scripts/Apply-ApprovedPatch.ps1` (motor) — aplica patch Git previamente aprovado somente aos caminhos explícitos em `-AllowedPath`, valida `diff --git` e cabeçalhos efetivos `---`/`+++`, roda `git apply --check` antes da escrita e não faz commit, push ou checkout. Dono: AGENTS.md. Validação: Test-ApplyApprovedPatchSelfTest.ps1.
