...
- `scripts/ClaudeCodeCliSupport.ps1` (support-library) — resolve `claude.exe` do PATH (versão mínima `2.1.118`), valida contrato de flags, **descarta ruído de ambiente do `stderr` antes de classificar** (avisos de `permissions.allow` descartadas, que aparecem inclusive em execução bem-sucedida), emite `max-turns-exhausted` para esgotamento de turno, detecta recusa por `workspace-not-trusted`, preserva a evidência bruta com instrução de coleta segura e classifica o status do job. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-ClaudeCodeCliSupportSelfTest.ps1. Tokens: `OK: Test-ClaudeCodeCliSupportSelfTest.ps1`.
