...
- `scripts/Invoke-GeneXusKbSpecifyGenerate.ps1` (motor) — specify/generate headless; bloqueia `DetailedNavigation=true` sem `-AllowCostlyBuildOptions`; mesmo bloqueio de environment (exit 46, `environment-validacao-indefinido`). Dono: xpz-msbuild-build/SKILL.md. Validação: via orquestrador; resiliência de pós-processamento por `Test-GeneXusMsBuildPostProcessingResilienceSelfTest.ps1` (`GENEXUS_MSBUILD_POST_PROCESSING_RESILIENCE_SELFTEST_OK`). Exit: 46.
