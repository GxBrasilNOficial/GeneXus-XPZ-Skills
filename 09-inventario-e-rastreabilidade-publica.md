...
- `scripts/Invoke-Antigravity.ps1` (motor) — backend Antigravity CLI síncrono (`agy -p`, `--mode plan`, `--output-format json`, `--print-timeout`, `--model`, resposta em `.response`; adapter argument-based fecha o stdin com `$null | & ...`; `-Message`/`-MessagePath` exclusivos + guard de tamanho fail-closed `$MaxArgvPromptChars`=30000; valida `$Mode = 'plan'`). Dono: xpz-llm-delegate/SKILL.md. Validação: via self-tests do backend.
