...
- `scripts/New-LlmDelegatePeerReviewArtifacts.ps1` (motor) — preparador transacional de artefatos da rodada: `-ManuscriptText` ou `-ManuscriptPath`, `ReviewersJson` com raiz array e `invokeArgs` objeto, staging `.prepare-*`, saída JSON única e nenhum dispatcher/adapter. Params TEST-ONLY: `-FailAt`. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-NewLlmDelegatePeerReviewArtifactsSelfTest.ps1. Tokens: `OK: Test-NewLlmDelegatePeerReviewArtifactsSelfTest.ps1`.
- `scripts/Invoke-LlmDelegatePanelDispatch.ps1` (motor) — harness MECÂNICO de despacho+coleta: aceita `-ManuscriptPath` ou `-ManuscriptText` (preparado antes de qualquer despacho por New-LlmDelegatePeerReviewArtifacts), valida `invokeArgs.backend` antes do dispatcher, despacha titulares em paralelo, registra `attemptRole`/`rank`/`fallbackChain`/`countsForDiversity`, classifica sentinelas explícitas de cota/saldo/limite como `quota`, ativa `fallbackChain` ordenado para `quota`/`timeout`/`error`/`unavailable`, usa o executável PowerShell atual/validado na invocação recursiva e mata processo filho de fallback ao exceder timeout, marca `skippedAfterSuccess`/`skippedByPolicy`/`notAttempted`, emite `panel-summary.json` + `manifest.json` + ledger; falha de preparação emite resumo estruturado sem iniciar rodada; NÃO injeta nativo, NÃO calcula piso/closeout/triagem/convergência (do orquestrador). Params TEST-ONLY: `-OpenCodeConfigPath`/`-CodexConfigPath`/`-BackendExeMap`. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1. Tokens: `OK: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1`.
