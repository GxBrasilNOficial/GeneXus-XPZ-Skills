...
- `scripts/Invoke-Antigravity.ps1` (motor) — backend Antigravity CLI síncrono (`agy -p`, `--mode plan`, `--output-format json`, `--print-timeout`, `--model`, resposta em `.response`; adapter argument-based fecha o stdin com `$null | & ...`; `-Message`/`-MessagePath` exclusivos + guard de tamanho fail-closed `$MaxArgvPromptChars`=30000; valida `$Mode = 'plan'`). Dono: xpz-llm-delegate/SKILL.md. Validação: via self-tests do backend (fake-exe) e medição direta contra o `agy.exe` instalado em 2026-08-04, com a saída literal registrada em xpz-llm-delegate/SKILL.md («Contrato JSON observado»): envelope `{conversation_id, status, response, duration_seconds, num_turns, usage{…}}`, mais `error` em falha; erro real do CLI vem com **exit code ≠ 0** (o guard de `status != SUCCESS` é defesa em profundidade, não o fluxo observado).
- `scripts/AntigravityCliSupport.ps1` (support-library) — resolve `agy.exe` do PATH ou `%LOCALAPPDATA%\agy\bin\agy.exe`, valida contrato de flags (`--print`/`--prompt` e `--mode`), extrai mensagens de erro e exporta regex `$quotaFailurePattern` para a suíte de autoteste e utilitários. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-AntigravityCliSupportSelfTest.ps1. Tokens: `OK: Test-AntigravityCliSupportSelfTest.ps1`.
