...
- `scripts/ClaudeCodeCliSupport.ps1` (support-library) — resolve `claude.exe` do PATH (versão mínima `2.1.118`), valida contrato de flags, detecta `workspace-not-trusted` e classifica o status do job. Dono: xpz-llm-delegate/SKILL.md. Validação: Test-ClaudeCodeCliSupportSelfTest.ps1. Tokens: `OK: Test-ClaudeCodeCliSupportSelfTest.ps1`.
...
- `scripts/Invoke-LlmDelegatePanelDispatch.ps1` (motor) — harness MECÂNICO de despacho+coleta: aceita `-ManuscriptPath` ou `-ManuscriptText` curto (preparado antes de qualquer despacho por New-LlmDelegatePeerReviewArtifacts; payload grande deve usar `-ManuscriptPath`), valida `invokeArgs.backend` antes do dispatcher, despacha titulares em paralelo, registra `attemptRole`/`rank`/`fallbackChain`/`countsForDiversity`, classifica sentinelas explícitas de cota/saldo/limite como `quota`, ativa `fallbackChain` ordenado para `quota`/`timeout`/`error`/`unavailable`, usa o executável PowerShell atual/validado na invocação recursiva e mata processo filho de fallback ao exceder timeout, marca `skippedAfterSuccess`/`skippedByPolicy`/`notAttempted`, emite `panel-summary.json` + `manifest.json` + ledger; falha de preparação emite resumo estruturado sem iniciar rodada; NÃO injeta nativo, NÃO calcula piso/closeout/triagem/convergência (do orquestrador). Params TEST-ONLY: `-OpenCodeConfigPath`/`-CodexConfigPath`/`-BackendExeMap`. Dono: xpz-llm-delegate/SKILL.md; também 15-revisao-por-pares.md. Validação: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1. Tokens: `OK: Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1`.
