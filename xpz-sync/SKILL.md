...
16. Reportar: objetos criados, atualizados, ignorados, resíduos removidos, refresh do índice e resumo Git
    ...
    - **`-BlockCrossFlowDataSource` (bloqueio opt-in):** aborta o sync (terminating error `ErrorId=CrossFlowDataSourceCollisionBlocked`, precedido da linha estável `CROSSFLOW_BLOCKED_ERRORID:` no stderr) **antes** de gravar qualquer metadata ou XML, em vez do fail-soft. Separado do fail-closed de pacote misto. **NÃO** impede a sobrescrita de XML existente **não-parseável** (tratado como moderno; Decisão G — falha seguro).
