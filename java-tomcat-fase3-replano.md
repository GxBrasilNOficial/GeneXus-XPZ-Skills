...
>
> Companheiro de `java-tomcat-paridade-gerador-design.md` (design congelado, sub-passos i–x), `evidence-catalog-java-tomcat.md` (fonte-verdade empírica da Fase 0 e da Fase 5) e `java-tomcat-fase3-checklist-implementacao.md` (checklist de implementação + Fase 5). Persistido no repo em 2026-07-05 (antes vivia só em scratchpad efêmero). **A implementação (Fase 3) segue esta spec congelada; a Fase 5 empírica foi registrada em 2026-07-07 para os ambientes disponíveis da KB Java externa. Novas medições seguem dependentes dessa KB porque a máquina de dev não tem licença Java.**
...
2. **Alvo externo declarado pelo environment**: `SERVLET_DIR` no `model.ini` do env apontou para `...\webapps\EBTECHPrototipo_18U14\WEB-INF\classes`. Pós-Fase 5, esse valor é tratado como fonte primária para preencher o metadata, **não** como prova isolada: quando houver Gradle, deve ser confrontado com `gradle.properties`/`TOMCAT_WEBAPP_PATH` e com as sentinelas do webapp antes de aceitar o alvo. Também existem `TOMCAT_PATH`, `CLIENT_STATIC_DIR`, `WebRoot`.
...
4. **Geração GeneXus é content-aware.** No-op DCE-eliminado não propaga nada; mudança real regenera → Gradle recompila → deploy publica no externo. **Confirmado que build incremental SEM mudança não toca o `WEB-INF\classes` externo** (Q5a) — logo `Pf=∅` num build rotineiro sem mudança (base do quadrante `no-evidence`). **Pós-Fase 5:** clean build dedicado confirmou `Rebuild All -> Lf=∅, Pf≠∅` para EBTECH/Jakarta (`Lf=0`, `Pf=624`), mantendo a política fail-safe do quadrante `unexpected-publication`.
...
9. **Dois sabores Java:** aferido `JAKARTA_EE`/`jakarta.*`; na Fase 5 também foi medido o environment GeneXus `JAVA_EE`/Gradle `javaEE` disponível, com sinais reais `javax`, mas em Tomcat 11/JDK 21/Servlet 6 e com jars `jakarta.*` também presentes. **Não aferido:** Java EE clássico puro Tomcat 8/9 + JDK 8. O sabor é atributo do **environment** (Tomcat/JDK por env), não da família — uma KB pode ter env Jakarta e env JavaEE (ver vi-bis). `allowlist com\<kb>` é namespace-agnóstica.
...
**B1 — metadata, campo dedicado** `kb_environment_servlet_dirs` (por env Java; fonte primária = `SERVLET_DIR` do `model.ini`). Coerente com decisão (b). Custo: `AUDIT_REQUIRED` no setup. **Validação de alvo antes de gravar metadata:** (a) termina em `WEB-INF\classes` (não `build\classes` local); (b) sibling `WEB-INF\lib\GeneXus.jar` existe; (c) raiz existe/é diretório; (d) reporta a raiz do webapp (pai de `WEB-INF`); (e) quando houver Gradle, confronta com `<output>\web\gradle.properties`/`TOMCAT_WEBAPP_PATH`; (f) o pacote declarado em `kb_environment_app_package` deve ser compatível com a app. Divergência → auditoria, não escrita cega. Falha → `unknown` (config-error), nunca `fresh`. **No motor**, `kb_environment_app_package` é allowlist da evidência; ausência de artefatos sob esse pacote aparece como `stale`/`no-evidence`/`unexpected-publication` conforme o quadrante, não como nova guarda de topologia. **Defesa em profundidade:** webapp estruturalmente válido porém **errado** passa (a)-(d) mas é pego pelo co-gate (`stale`).
...
**Política do `unexpected-publication` (declarada, correção v5 — achado glm A1):** na spec congelada, o quadrante `∅,≠∅` foi tratado como **política fail-safe** ("origem não atestável", **não** "deploy necessariamente errado"), com **mensagem acionável** enumerando causas prováveis (rebuild forçado sem regeneração / cópia manual / deploy externo) + remédio ("rode geração antes do deploy, ou marque diagnóstico"). **Pós-Fase 5:** clean build dedicado em EBTECH/Jakarta confirmou `Rebuild All -> Lf=∅, Pf≠∅` no fluxo nominal (`Lf=0`, `Pf=624`). O comportamento confirma que o quadrante ocorre; a política permanece conservadora nesta frente. Relaxar a classificação exigiria nova decisão de design (por exemplo, um estado de "última publicação bem-sucedida"), não inferência automática a partir da medição.
...
**Âncora do limite superior — preferência de design (correção v6):** a âncora **preferida-por-design** de `threshold_sup` é o **fim do passo de publicação** — `DeployStepCompletedAt` (o instante em que o deploy termina de copiar ao `WEB-INF\classes`, se observável no bloco `timing` do MSBuild Java) `+ slack`. O `BuildEndedAt + slack` é o **fallback explícito** quando `DeployStepCompletedAt` não estiver disponível. Esta ordem importa: se a publicação for **posterior** ao `BuildEndedAt` (deploy assíncrono/demorado, ou sub-passo cujo timestamp o MSBuild não fecha em `BuildEndedAt`), um `.class` **legítimo** cairia fora de `[threshold_inf, threshold_sup]` sob o fallback → **falso-stale**. Usar o fim-de-publicação como âncora primária evita isso; o fallback só vale para topologia same-host onde deploy é passo síncrono do BuildAll (o caso aferido, skew≈0). **Pós-Fase 5:** a publicação observada acontece via Gradle/tarefas `copyTomcat*`; o marco útil é o fim da cópia/publicação, não o fim de `compileJava`.

**Ator da cópia (correção v7 — achado glm R1; atualizado pós-Fase 5):** a Fase 0 estabeleceu a topologia externa (webapp no Tomcat, fora da árvore de output), mas ainda não havia aferido **quem** copia para o `WEB-INF\classes`. A Fase 5 fechou o ator para EBTECH/Jakarta: Gradle chamado pelo Build All GeneXus/MSBuild, por tarefas `copyTomcat*` (`copyTomcatClasses`, `copyTomcatLib`, `copyTomcatWebInf`, etc.). Ponto que sustentava o congelamento e continua válido: **em qualquer dos casos o sentido de falha é conservativo** — quando a âncora superior fica cedo demais, um `.class` legítimo cai **acima** da janela → excluído de `Pf` → `stale`/`no-evidence`, **nunca** falso-`fresh`. Logo a incerteza sobre observabilidade do marco exato não viola a propriedade de segurança; só pode gerar ruído (falso-stale).
...
- **Campo de config:** `deployBinTimeSlackSeconds` no registro `GeneXusKbHostingKindSupport.ps1`, default **5** (coerente com a disciplina metadata-driven; herdado do .NET). **Pós-Fase 5:** `5s` cobre a janela interna de cópia observada (`copyTomcatClassesStartAt -> lastPublishedClassMTime = 1,824s`), mas não o fim de `compileJava` como âncora (`compileJavaEndAt -> lastPublishedClassMTime = 24,045s`); portanto o marco correto continua sendo fim da publicação/cópia.
...
- **Marcação de proveniência empírica por-env:** `certified` | `not-audited` (o env aferido EBTECH/Jakarta = `certified`; pós-Fase 5, o env GeneXus `JAVA_EE`/`javaEE` disponível também foi medido com sinais `javax`, mas no stack Tomcat 11/JDK 21/Servlet 6; Java EE clássico puro Tomcat 8/9 + JDK 8 segue `not-audited`).
...
- **Fase 5 (gêmeos empíricos Java) = frente própria**; precisou da KB Java da colega e foi registrada em 2026-07-07 para os ambientes disponíveis. Resultado: (i) clean build confirmou `Rebuild All -> Lf=∅,Pf≠∅` em EBTECH/Jakarta; (ii) latência mostrou que `5s` cobre a cópia, não `compileJava` como âncora; (iii) environment GeneXus `JAVA_EE`/`javaEE` disponível medido com sinais reais `javax`, mas não Java EE clássico puro Tomcat 8/9 + JDK 8; (iv) ator da cópia confirmado como Gradle/tarefas `copyTomcat*`; (v) conjunto fechado `{_impl, __default, __gam}` mantido conservadoramente, com `_bc`, `ww*`, `Sdt*`, `StructSdt*` e `StructSdtCol*` como stems próprios. O fail-safe do (v-bis) (sufixo desconhecido → objeto próprio) continua protegendo na direção segura.

## 5. Fora de escopo da Fase 3
Motores Eixos B/C em Java = Pós-v1; `.war` = aterramento próprio; migração de alvo .NET→Java = decisão humana; §9 por AST = endurecimento futuro (`999`); âncora de tempo do deploy para skew grande = follow-up (`999`) após a Fase 5 ter confirmado a direção empírica da cópia/publicação.
