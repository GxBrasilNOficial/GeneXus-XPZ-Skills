...
- `scripts/Test-GeneXusFrontAcervoDrift.ps1` — gate `9-FD` (drift frente-vs-acervo: compara `lastUpdate` de cada XML da frente contra o homônimo no acervo; findings `front-older-than-acervo` bloqueiam empacotamento; findings `front-object-type-drift` bloqueiam empacotamento quando o mesmo `guid` diverge em `Object/@type`, exigindo decisão humana; `front-object-type-drift-ambiguous-acervo` é diagnóstico informativo de GUID duplicado no acervo; proteção mecânica parcial nesta versão: bloqueia remoção forte de trailing whitespace herdado contra baseline por GUID único, mas não detecta introdução de trailing whitespace novo nem é detector completo de whitespace, reindentação ou EOL; `front-textual-fidelity-info` é informativo apenas quando havia baseline único por GUID, mas a leitura/decodificação UTF-8 comparável falhou)
- `scripts/Copy-GeneXusAcervoToFront.ps1` — motor de resolução de drift temporal/seed e reconstrução textual por alvo explícito: copia XMLs do acervo para a frente com bump automático de `lastUpdate`; com `-ObjectList`, `-ObjectNames` ou `-ObjectGuids`, também pode reconstruir/sobrescrever a frente mesmo quando ela está mais nova que o acervo para remediar `front-textual-fidelity-trim-removal-churn`; alvo explícito sobre objeto já existente/editado deve ser deliberado e preferencialmente precedido de `-DryRun`; resolve o anti-padrão "editar acervo esperando que o pacote pegue"; bloqueia autocópia quando o mesmo `guid` diverge em `Object/@type` (`front-object-type-drift-skip`)
