...
- A primeira alteração normativa que substitui a regra antiga deve ser aplicada ainda pela regra vigente no momento da edição, usando `xpz-codex-apply-patch-alternative/SKILL.md` e `scripts/Apply-ApprovedPatch.ps1`, salvo autorização humana textual específica para superar essa transição.
