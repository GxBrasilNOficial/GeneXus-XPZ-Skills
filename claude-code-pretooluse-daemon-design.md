...
- **0e. Critério de veredito (RE-ENQUADRADO pós-passo-0 — substitui o orçamento absoluto da v4; base
  ...
  - **NOTA PÓS-PASSO-F (reconciliação, 2026-06-30 — decisão datada do autor; fonte: `historico/passoF-daemon-pretooluse-medicao-20260630/relatorio-passoF-v1.md`):** a medição do **FIO REAL** (cliente NativeAOT + daemon pwsh já buildados, **não** o protótipo do passo 0) deu overhead-sobre-o-piso **~20–34 ms** (cross-check perc-a-perc ~19–30 ms até p95; pareado p90 ~32–34 ms; n=1000, 0 mismatch) — portanto o teto de **5 ms do (i) REPROVA**. O protótipo do passo 0 que calibrou o (i) era **não-representativo** (não computava identidade do cliente, ~5,5 ms; daemon mais enxuto, sem a matriz de staleness completa nem a dupla desserialização, ~17–24 ms/requisição). **Decisão do autor:** o **(i) é RECLASSIFICADO de "GATE de liberação" para "ALVO de otimização"**; o fio é **liberado no mérito** porque o baseline operativo real **não** é "spawn + 5 ms", e sim o **clique humano (~1.500–10.000 ms)** e o **hook pwsh atual (~466 ms)**, contra os quais o daemon a **~50 ms e2e, fail-closed (nunca `allow` isolado), 0 mismatch** é **~9–200× melhor**. Os gates **(ii) guardrail de cauda** e **(iii) não-regressão CONTINUAM valendo como condição de liberação e passam**; a segurança (§8 verde) não é tocada. Reduzir o ~17–24 ms do daemon em PS por requisição (evitar a dupla desserialização, enxugar a staleness no caminho quente, ou mover o laço quente para fora do PowerShell) é **desdobramento desejável, NÃO condição de liberação**. O autor **dispensou o painel** para esta decisão (mérito evidente; o ponto é a má-formulação do (i), não a leitura de um número de borda) — registrado para auditoria; a porta para revisão por pares posterior da tese (§11 do relatório do Passo F) permanece aberta.
