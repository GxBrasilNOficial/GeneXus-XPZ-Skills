...
8.g2.ii Para cada diretório presente, ler pelo menos um XML e extrair o tipo canonico:
    ...
    - Elemento legado órfão: se o `Object/@type` casar `gxlegacy/<Elemento>` (sem GUID), mapear pelo registro `scripts/gx-legacy-export-element-registry.json` (classe `orphan` → `materializedFolderName`), não tratar como tipo desconhecido — o motor `Test-XpzObjetosDaKbNaming.ps1` já faz isso. Pastas órfãs (`Report`/`Menubar`) só aparecem quando o motor de export legado (frente futura) as materializar
