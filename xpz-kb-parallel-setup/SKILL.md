...
- Gravar, para env Java/Tomcat, os campos de metadata dedicados do alvo do co-gate (Eixo A): `kb_environment_servlet_dirs` (= `SERVLET_DIR` do `model.ini` do env, termina em `WEB-INF\classes`), `kb_environment_app_package` (pacote da app, ex.: `com\<kb>`) e `kb_environment_servlet_flavor` (`jakarta`|`javax`). São **AUDIT_REQUIRED** (autoria declarada pelo usuário; `model.ini`/`GeneratorType` nunca autoridade). "Gravar" aqui = **escrita manual** no `kb-source-metadata.md`: **não há setter/auditoria dedicados** para esses 3 campos hoje — `Set-XpzKbSourceMetadataDeployment.ps1`/`Test-XpzKbMetadataWrapper.ps1` **não** os escrevem/validam (por design). Sem eles, o co-gate Java resolve config-error → `unknown` (fail-safe), nunca `fresh`. A populacao automatica + auditoria pelo setup a partir do `model.ini` é follow-up (Fase 5, gated pela KB Java)
