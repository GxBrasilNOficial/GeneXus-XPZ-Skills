...
- Quando o empacotamento local com `import_file.xml` for recorrente, recomendar wrapper local fino para criacao do pacote, por exemplo `New-*KbImportPackage.ps1`:
  ...
  - pode receber `ReportPath` absoluto, novo e `.json`; o destino deve ter pasta pai existente, não atravessar reparse point e ficar fora de acervo, frente, `XpzExportadosPelaIDE`, scripts, `KbIntelligence`, `.git`, `ArquivoMorto`, metadata e artefatos do pacote; caminho invalido retorna `stage=validate-report-path`, `exitCode=20` sem criar pacote ou relatorio; com `ReportPath` válido, o pós-inventário do wrapper compartilhado liga fail-closed (`FailOnDeltaMismatch` + `FailOnUnknownTypes`) e, se o inventário devolver exit ≠ 0, o stdout pode sair `status=bloqueado` e o exit do empacotamento muda — sem `ReportPath`, o inventário corre em modo informativo e não promove esse exit
