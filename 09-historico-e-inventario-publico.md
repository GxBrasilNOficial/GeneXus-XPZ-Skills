...
- `Inferência forte`: com evidência operacional repetida em `Teste1`, `GxTeste2`, `GxTest3` e `TravelAgency`, a trilha central de exportacao, preview e importacao via `MSBuild` passou a ter repetibilidade suficiente para ser tratada como mecanismo experimental validado; os bloqueios remanescentes relevantes desta frente ficaram concentrados em conteudo especifico de KB/`XPZ`, nao no wrapper.
