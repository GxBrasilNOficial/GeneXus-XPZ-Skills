...
- A revisão pré-push desta rodada apontou quatro gaps de documentação: ausência de regra operacional no `02`, ausência da entrada equivalente no `08`, ausência do motor em `xpz-builder/SKILL.md` e no `quality-checklist.md` (com o «Dono» do ponteiro do `09` apontando um documento de desenho em vez de uma skill) e ausência desta entrada no histórico mensal. O `02`, o `08` e esta entrada foram corrigidos no commit seguinte; a definição do dono normativo ficou como decisão humana, tomada em seguida a favor de `xpz-builder/SKILL.md` — documento congelado não acompanha o motor, e o `09` diz que o detalhe de contrato vive no dono.
- Uma revisão externa posterior, por outro agente, apontou seis gaps **de motor**, todos confirmados contra o código e corrigidos no commit seguinte: (1) `-ReportPath` sem a família de guardas dos demais caminhos — sem exigência de `.json`, sem área protegida, sem ponto de reanálise, substituindo destino existente de qualquer natureza; (2) sumiço de arquivo entre as fases virando erro interno (alvo) ou silêncio (acervo consultado) em vez de `PLAN_STALE`; (3) roteiro de recuperação sem instrução para renome interrompido entre `started` e `committed`, justamente o que sustenta a recusa de adiar `renameDomain` na §14; (4) XML de `PackagedModule` ilegível engolido por `catch { continue }` no ramo que existe para ser fail-closed; (5) fixture do scanner sintético onde a §13.7 exige o real `GAMMessageType`; (6) ausência de cobertura de renome interrompido. A conferência local acrescentou um sétimo, da mesma classe do (2) e não apontado pela revisão: `File.Copy` do `.bak` na Fase 1b também lançava com o alvo ausente — era a repetição do padrão de «fechar um eixo e deixar o vizinho aberto» que a §5 do `13` nomeia. Os casos 9 e 13 passaram a usar gatilho observável; o 9, com espera cega, quase nunca disparava e passava sem provar nada.
- Uma segunda rodada da mesma revisão externa apontou mais dois: o `08` ficou sem a ressalva do `-ReportPath` que já estava no `02`, no `09` e na skill (defasagem de três documentos corrigidos e um esquecido), e a bateria não tinha **nenhuma** ocorrência de `PackagedModule`, embora o desenho exija cobertura em dois pontos. Entraram os casos 7.6 a 7.8 e, por conferência local, o 2b — Part de documentação dentro de `<Object>` aninhado não conta para a âncora, que é a regra central da §6.0 e estava sem prova. Durante a escrita desses fixtures, dois deles saíram deformados pela precedência da vírgula sobre o `+` em array literal do PowerShell: o XML continuava bem formado, o atributo virava `"\nGAMMessageType\n"`, o detector não casava e o caso passava em branco. Fixture também é código e precisa ser conferido contra o que diz representar.
- A pré-push seguinte, já sobre os cinco commits, achou o mesmo padrão de eixo vizinho pela terceira vez: o ramo silencioso corrigido em `Test-GeneXusPackagedModuleHomonym` tinha três irmãos (`Get-GeneXusDomainDefinitionIndex`, `Get-GeneXusAcervoTypeFolderIndex`, `Get-GeneXusModuleNameByGuid`, `Get-GeneXusFolderObjectIndex`), dois deles **fail-open**. Fechados no commit seguinte, junto com o `-WorkDir` que não recusava ponto de reanálise, e com a cobertura que faltava: `-AcknowledgeReferences` e o **rollback do rename** — a única promessa da §14 cuja execução nunca tinha sido testada, só o roteiro.
