...
- `GeneXusMsBuildStderrNoiseSupport.ps1`: implementado como helper comum para separar ruído estrutural comprovado de stderr (`stderrFilteredNoise`) de erro real (`stderrContent`) nos wrappers de build e nos wrappers headless desta skill que expõem esse campo; pares cruzados ou linhas não comprovadas permanecem como stderr real
...
- `GeneXusMsBuildStderrNoiseSupport.ps1`
  - status atual: implementado
  - objetivo: helper comum para filtrar somente os ruídos estruturais comprovados do GeneXus 18 nos diagnósticos `stderrFilteredNoise`, preservando erros reais em `stderrContent`; consumido por import/export/preview e demais wrappers headless desta skill
