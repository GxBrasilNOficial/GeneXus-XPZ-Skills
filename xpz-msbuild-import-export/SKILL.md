...
- **Proteção upstream:** o empacotamento por `New-XpzImportPackage.ps1` roda o gate de drift 9-FD **sempre** (fail-closed); `Build-GeneXusImportFileEnvelope.ps1` roda o gate de `lastUpdate` **sempre**, com `-AcervoPath` obrigatório. Em ambos, `lastUpdate` velho ou igual ao acervo em objeto declarado modificado **bloqueia** antes da gravação do pacote.
