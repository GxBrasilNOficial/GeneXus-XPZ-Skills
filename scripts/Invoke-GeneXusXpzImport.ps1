...
.PARAMETER InputPath
Caminho do pacote de importação de entrada. Nome canônico do contrato de entrada
primária; aliases aceitos: `-XpzPath` (retrocompatível) e `-Path`. Aceita `.xpz` (formato compactado
...
if ($logPathRejection.rejected) {
    Write-Output (New-GeneXusMsBuildLogPathBlockJson -WrapperName 'Invoke-GeneXusXpzImport.ps1' -ResolvedLogPath $resolvedLogPath -Reason $logPathRejection.reason)
    exit 50
}
...
try {
    ...
    try {
        if (Test-Path -LiteralPath $signalsScript -PathType Leaf) {
            $signalsJson = & $signalsScript -StdOutPath $stdOutPath -StdErrPath $stdErrPath -ExpectedItems ($includeItemsArray -join ';') -Stage 'import-real' -OutputPath $signalsPath -AsJson
