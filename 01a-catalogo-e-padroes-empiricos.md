...
- `Regra operacional`: a classificação do override local é triádica por entrada: `pending` quando a entrada ainda precisa subir para o catálogo base; `redundant` quando o base já cobre a mesma informação; `divergent` quando o override contradiz o base e exige decisão humana. `pending` é lembrete efetivo, `redundant`/metadata-only recomenda limpeza com `CLEANUP_RECOMMENDED`, e `divergent`/shape inválido/colisão insegura bloqueia a resolução do catálogo efetivo.
