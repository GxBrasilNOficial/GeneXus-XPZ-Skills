...
- **legacy GeneXus 9 export element**: a GeneXus 9 `ExportFile` carries `<GXObject><Element>` by **name**, with no type GUID — it stays **outside** the modern catalog, in the registry `scripts/gx-legacy-export-element-registry.json` (owner doc `01k`), which classifies each element as `equivalent` (real modern type) or `orphan` (`typeToken` `gxlegacy/<Element>`); the legacy export engine is a future workstream.
