...
- ao usar `New-XpzImportPackage.ps1`, o gate de drift 9-FD (`Test-GeneXusFrontAcervoDrift.ps1`) executa sempre antes do empacotamento (fail-closed) e bloqueia XMLs na frente com `lastUpdate` mais antigo que o homonimo no acervo ou com mesmo `guid` e `Object/@type` divergente (`front-object-type-drift`); `-AcervoPath <ObjetosDaKbEmXml>` e opcional e, quando omitido, o acervo canonico `<RepoRoot>/ObjetosDaKbEmXml` e resolvido automaticamente; sem acervo resolvivel o empacotamento e bloqueado, e o JSON reporta `acervoResolvedBy` (`explicit` ou `convention`); para drift de `Object/@type`, parar para decisao humana em vez de autocopiar/bump; para seed inicial de objeto ainda ausente na frente, usar `Copy-GeneXusAcervoToFront.ps1` com `-ObjectList`, `-ObjectNames` ou `-ObjectGuids` explicito; a pasta da frente deve já existir, aberta por `New-GeneXusXpzFront.ps1` (wrapper `New-*KbFront.ps1`, `-ReuseIfExists` para retomar), nunca criada manualmente — `Copy-GeneXusAcervoToFront.ps1` e os gates 9-* apenas populam ou inspecionam uma frente existente
- `-TemplatePackagePath` em `Build-GeneXusImportFileEnvelope.ps1` e `New-XpzImportPackage.ps1`/`.py` aceita tanto `.import_file.xml`/XML quanto `.xpz` real comparavel; quando o template traz `Attributes` de topo e a frente não traz raizes `Attribute` explicitas, o motor preserva esses `Attributes`
...
- al usar `New-XpzImportPackage.ps1`, el gate de drift 9-FD (`Test-GeneXusFrontAcervoDrift.ps1`) se ejecuta siempre antes del empaquetado (fail-closed) y bloquea XMLs en el frente con `lastUpdate` más antiguo que el homólogo en el acervo o con el mismo `guid` y `Object/@type` divergente (`front-object-type-drift`); `-AcervoPath <ObjetosDaKbEmXml>` es opcional y, cuando se omite, el acervo canónico `<RepoRoot>/ObjetosDaKbEmXml` se resuelve automáticamente; sin acervo resoluble el empaquetado queda bloqueado, y el JSON reporta `acervoResolvedBy` (`explicit` o `convention`); para drift de `Object/@type`, detenerse para decisión humana en vez de autocopiar/bump; para seed inicial de un objeto aún ausente en el frente, usar `Copy-GeneXusAcervoToFront.ps1` con `-ObjectList`, `-ObjectNames` o `-ObjectGuids` explícito; la carpeta del frente debe ya existir, abierta por `New-GeneXusXpzFront.ps1` (wrapper `New-*KbFront.ps1`, `-ReuseIfExists` para retomar), nunca creada manualmente — `Copy-GeneXusAcervoToFront.ps1` y los gates 9-* solo pueblan o inspeccionan un frente existente
...
- when using `New-XpzImportPackage.ps1`, the 9-FD front-acervo drift gate (`Test-GeneXusFrontAcervoDrift.ps1`) always runs before packaging (fail-closed) and blocks XMLs in the front whose `lastUpdate` is older than the matching file in the acervo or whose same `guid` has divergent `Object/@type` (`front-object-type-drift`); `-AcervoPath <ObjetosDaKbEmXml>` is optional and, when omitted, the canonical acervo `<RepoRoot>/ObjetosDaKbEmXml` is auto-resolved; with no acervo resolvable, packaging is blocked, and the JSON reports `acervoResolvedBy` (`explicit` or `convention`); for `Object/@type` drift, stop for human decision instead of auto-copying/bumping; for initial seed of an object still absent from the front, use `Copy-GeneXusAcervoToFront.ps1` with explicit `-ObjectList`, `-ObjectNames`, or `-ObjectGuids`; the front folder must already exist, opened by `New-GeneXusXpzFront.ps1` (wrapper `New-*KbFront.ps1`, `-ReuseIfExists` to resume), never created manually — `Copy-GeneXusAcervoToFront.ps1` and the 9-* gates only populate or inspect an existing front
