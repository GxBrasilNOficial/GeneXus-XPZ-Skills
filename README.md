...
- **objetos gerados por Pattern x autorais no índice**: a partir de `schema_version=4`, o índice materializa na tabela `objects` se um objeto foi gerado por Pattern (WorkWith For Web) — colunas `is_generated_object` (0/1), `pattern_object_id` e `instance_key`, extraídas do `<Properties>` de nível-objeto (escopadas ao objeto raiz, nunca de `<Object>` aninhado em `PackagedModule`); usar o sinal determinístico e o filtro `--generated`/`--authored` (em `search-objects`/`list-by-type`) em vez de heurística de nome (`ww*`/`*WC`/`*General`/`*Selection`) ao separar autoral de gerado; ver `02`, `08` e `scripts/README-kb-intelligence.md`
...
- **objetos generados por Pattern vs autorales en el indice**: a partir de `schema_version=4`, el indice materializa en la tabla `objects` si un objeto fue generado por Pattern (WorkWith For Web) — columnas `is_generated_object` (0/1), `pattern_object_id` e `instance_key`, extraidas del `<Properties>` de nivel-objeto (acotadas al objeto raiz, nunca de `<Object>` anidado en `PackagedModule`); usar la señal determinista y el filtro `--generated`/`--authored` (en `search-objects`/`list-by-type`) en vez de heuristica de nombre (`ww*`/`*WC`/`*General`/`*Selection`) al separar autoral de generado; ver `02`, `08` y `scripts/README-kb-intelligence.md`
