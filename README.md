...
- `xpz-llm-delegate`: delegação de tarefas menores ou segunda opinião a um LLM secundário via opencode (modelos no formato `provider/modelo`, incluindo provedores cloud conhecidos como `ollama-cloud/*` e `opencode-go/*`), Codex (`codex exec`), Claude Code (`claude -p`, Opus 4.8), GitHub Copilot CLI (`copilot -p`), Gemini CLI (`gemini -p`) ou Antigravity CLI (`agy -p`), com classificação local/externo determinística e gate de confidencialidade por KB (chave de destino, nunca o adapter); acionamento sempre humano (inclui condução de revisão por pares multi-modelo, ver `15-revisao-por-pares.md`)
