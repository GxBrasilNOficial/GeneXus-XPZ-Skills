...
No tier **reforçado** ([`14`](14-revisao-pre-push-reforcada.md)), nem todo revisor consegue **executar** este passo mecânico: os adapters de LLM rodam os backends em modo restrito, e há uma classe **semantic-only** que **não tem shell** (não roda git nem `pwsh`) — o opencode `reviewer-ro` (nega `bash`) e, por configuração de adapter, Claude Code, Copilot, Gemini e Antigravity no perfil fixo `public-review`. A classe **git-capable** (roda git) é o Codex-delegate (sandbox read-only) e o subagente nativo (Bash). Antigravity nunca conta como git-capable; o `public-review` recusa `-Cd`, cria scratch temporário sem `.git`/reparse point/raiz de repositório/raiz de KB e redireciona o ambiente somente no processo filho. Duas capacidades, provadas empiricamente por probe, não inferidas.
...
**Eixo do transporte.** Adapters `stdin-dossier-capable` (opencode, Claude Code) recebem o dossiê por stdin, sem teto prático. Adapters `argv-limited` (Copilot, Gemini, Antigravity) passam o prompt por argv, com guard fail-closed (~30000 chars): dossiê acima do teto ⇒ o orquestrador **omite** o revisor (`unavailable`+`dossier-too-large`), sem truncar nem degradar em silêncio. Para Antigravity, somente payload `public` chega ao adapter: o gate roda primeiro e preserva `gateAsk`/`gateDeny`; se devolver `allow` para `kb-sensitive`, a postura fixa bloqueia antes do adapter como `unavailable` + `refusedSensitivity`. A classificação continua pertencendo ao gate/orquestrador, não ao adapter. O recibo `public-review` registra versão do CLI, workspace, redirecionamento de ambiente e limpeza; o keyring global do sistema não é isolado e essa limitação deve permanecer explícita.
...
**Lockstep de contrato de consumo de motor compartilhado.** Quando a frente altera de forma *breaking* o **contrato de saída/consumo** de um motor compartilhado (formato do stdout, shape do objeto de retorno, semântica de exit) consumido por **wrappers locais** (ex.: clones `Update-*KbFromXpz.ps1` em pastas paralelas) ou outros consumidores, conferir explicitamente que a skill que **audita/policia esses consumidores** (ex.: `xpz-kb-parallel-setup`, via `scripts/Test-XpzWrapperInventory.ps1` e os sinais `INVENTORY_*`) ganhou o **check de detecção de drift correspondente na MESMA frente** — não basta a doc da skill estar alinhada. Parquear a migração do consumidor só no `CHANGELOG`/`999` é insuficiente: o gate que existe para pegar consumidor defasado continua cego. Precedente: o motivo `consumes_legacy_text_stdout` de `INVENTORY_CUSTOMIZED` (para `Update-*KbFromXpz.ps1` que ainda consome o stdout textual do `Sync-GeneXusXpzToXml.ps1`).
