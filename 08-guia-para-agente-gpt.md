...
4. usar `01-base-empirica-geral.md` como índice mestre da serie `01` e abrir o filho empirico mais aderente (`01a` a `01h`, filhos empíricos; `01k` é doc de governança fora desse mapa)
...
- o gate consultivo `Test-PrePushNewTokenPropagation.ps1`, chamado pelo orquestrador mecanico, apoia a regra simetrica do passo 2 do `13`: detecta no diff termo de contrato introduzido por transicao co-localizada (ex.: novo `-ObjectList` ao lado de `-ObjectNames`/`-ObjectGuids`) e lista mencoes versionadas que ficaram só com o conjunto antigo; findings são candidatas consultivas em `agentWarnings`, não substituem a busca semantica e não disparam sem transicao co-localizada no diff
- gate novo exige enumerar os eixos vizinhos (do `13`, §5): ao introduzir bloqueio/recusa/validação (`Stop-WithReason` com motivo novo, `throw "BLOCK: …"`, `ValidateSet`/`ValidateRange` novo), listar por quais OUTROS caminhos o mesmo defeito ainda chega e dar veredito por eixo (fechado | aberto com justificativa | residual). Eixos recorrentes: motor par escritor↔leitor, outro parâmetro que alcança o mesmo estado, outra grafia da mesma chave, titular vs elo de `fallbackChain[]`, `public` vs `kb-sensitive`. Fechar um eixo e não varrer os irmãos é gap de completude, não «fora do escopo»
- disciplina de confronto por classe (do `13`): cada candidata do gate de propagacao recebe `mentionClass` (`prose`/`param-list-item`/`param-table-cell`/`command-example`); candidatas de `prose` admitem justificativa coletiva, mas as nao-prosa NÃO — o orquestrador as segrega no campo `nonProseVerdictRequired` e exige no `agentSemanticChecklist` um livro-razao item a item (cada uma com arquivo:linha, a lista/tabela/exemplo gemeo no outro documento e veredito gap|justificado). O gate não trunca candidatas nao-prosa (só a `prose` respeita `-MaxFindings`); o orquestrador projeta `candidateCount`/`classCounts`/`truncated`/`truncatedProseCount` e avisa quando houve truncamento, para o revisor nao ler a lista de candidatas como exaustiva. Acima do limiar de nao-prosa (hoje > 5), o checklist recomenda uma segunda passada da fase semantica por modelo distinto: forcar exaustividade num único modelo confronta mas não garante veredito correto (ele pode racionalizar cada item como justificado), e a diversidade de modelo e o backstop
...
- regra de lockstep do passo 3 do `13`: quando a frente altera de forma *breaking* o contrato de consumo de um motor compartilhado (formato do stdout, shape do retorno, semantica de exit) consumido por wrappers locais (ex.: `Update-*KbFromXpz.ps1` em pastas paralelas), conferir na fase semantica se a skill que **audita os consumidores** (ex.: `xpz-kb-parallel-setup` via `scripts/Test-XpzWrapperInventory.ps1`) ganhou o check de deteccao de drift no mesmo PR; paridade documental da skill não basta. Precedente: o motivo `consumes_legacy_text_stdout` de `INVENTORY_CUSTOMIZED`
...
- o gate consultivo `Test-PrePushBackendEnumerationParity.ps1`, chamado pelo orquestrador mecanico, deriva de `$AdapterScript` em `scripts/Invoke-LlmDelegatePanelDispatch.ps1` o conjunto de backends canonicos de delegação (#1 opencode, #2 codex, #3 claude-code, #4 copilot, #5 gemini, #6 antigravity) e sinaliza unidades logicas na doc que ficaram como subconjunto próprio sem justificação/marcador de ignore; findings são candidatas consultivas em `agentWarnings`
...
- a rotina pre-push acima (documento `13`) é a do **repositório de skills**; a validação pré-push do estado de uma **pasta paralela de KB** (antes do push dessa KB) é outra rotina — a skill `xpz-kb-parallel-pre-push` (orquestrador `Invoke-XpzKbParallelPrePushPhase1.ps1`, gates mecânicos próprios; Fases 2a/2b/2c — a 2c é triagem de agente no relatório, não gate e não prova ausência de regressão) — com autoridade documental distinta; não confundir os dois contextos
- termo reservado: quando o usuário pedir `revisão por pares`, `peer review`, `painel multi-modelo` ou `validar plano multi-modelo`, o agente deve primeiro ler `xpz-llm-delegate/SKILL.md` e `15-revisao-por-pares.md` (e, no caso pré-push reforçado, também `14` + `13`); se não houver lista preferida resolvida (`Resolve-LlmDelegatePreferredReviewers.ps1` com `-Orchestrator`; cascata `preferred-reviewers.<orquestrador>.json` → machine), perguntar quais ferramentas/modelos o usuário tem disponíveis ou prefere (`Claude Code`, `opencode/Ollama Cloud`, `Codex`, `Copilot`, `Gemini`, `Antigravity`, subagente nativo) antes de oferecer painel, sem presumir assinatura externa; depois que o usuário escolher revisores para a rodada, oferecer salvar essa seleção como curadoria (`preferred-reviewers.json` machine ou `preferred-reviewers.<orquestrador>.json`), separadamente da autorização por KB em `llm-delegation-policy.json`; antes de gravar, rodar `Resolve-LlmDelegatePreferredReviewers.ps1 -Orchestrator <harness>` e ler `preferenceSource`/`effectivePreferredPath`/`cascadeOrchestratorExists`/`cascadeMachineExists`: pedido genérico de editar a lista preferida nesta sessão vai para `-Scope orchestrator` quando o ficheiro do orquestrador já existe ou é a fonte efetiva, e `-Scope machine` só com pedido explícito de lista da máquina/todos os harnesses — gravação machine com `preferred-reviewers.<orquestrador>.json` presente não muda o que este harness resolve, e o `Set-` sinaliza isso com `harnessResolveReadsThisPath=false` e `diagnostics` `machine-write-shadowed-by-orchestrator-file`, que devem ser mostrados ao usuário em vez de anunciar "lista deste harness atualizada"; se houver lista preferida resolvida, tratar como candidatos preferidos do painel (não pool opcional): no schema v3, titulares CLI (`delegation-cli`) e nativo (`orchestrator-native-subagent`); `rank`, `reasoningEffort`, `invokeArgs` obrigatório; CLI pode ter `fallbackChain[]` por titular (nativo proíbe cadeia); fallback só entra quando o titular falha/indisponibiliza, todos os titulares/fallbacks aplicáveis precisam de estado auditável, pulados/não tentados carregam `countsForDiversity=false`, `noResponse` é reclassificação pós-hoc de resposta sem parecer útil, e `insufficientDiversityAfterFallback` impede usar o rótulo `revisão por pares`; rodar gate por preferido e diversidade sobre a lista completa de candidatos preferidos + vereditos, sem parar no mínimo de 2 famílias por decisão própria; antes do recibo final, rodar `Resolve-LlmDelegatePeerReviewCloseout.ps1` com `-PreferredReviewersSnapshotJson` quando havia preferidos (ao autorar a versão consolidada vN+1, passar `-VNextState pendingResubmission`, que bloqueia até `resubmitted` ou declínio auditado por `-ResubmissionDeclinedBy`/`-ResubmissionDeclineReason`/`-RoundId`) e, se `closeoutReady=false`, apresentar o `requiredUserPrompt` em vez de encerrar a rodada; é proibido chamar parecer solo de revisão por pares — sem painel válido com >=2 famílias efetivamente consultadas, rotular como `parecer solo` ou `segunda opinião (N)`; antes de usar o rótulo, apresentar recibo mínimo (arquivos lidos, manuscrito/prompt, revisores, famílias, piso de diversidade, vereditos, estado da vN+1 (`vNextState`), estado de cada revisor preferido quando houver lista, `preferenceSource`/`effectivePreferredPath` e adendo de closeout quando aplicável); resposta em menos de 30s desde o pedido é incompatível com revisão por pares real, salvo relatório de painel anterior identificável; quando a arquitetura do plano estabiliza e só restam objeções de **precisão de implementação** (não gaps de design), o agente pode **propor** congelar o design e transferir a prova para implementação/self-test — a **decisão** de congelar é humana e registra-se no closeout como `resubmissionDeclinedByHuman` com motivo "prova transferida para implementação/self-test", **não** `resubmitted` (o papel não passou; a prova mudou de meio); ver `15-revisao-por-pares.md`, seção `## Quando o design estabiliza: congelar o papel e migrar para o código`
- tier reforcado (opcional): revisao por painel multi-modelo diverso, em `14-revisao-pre-push-reforcada.md` (aplicacao pre-push da metodologia generica de revisao por pares do `15-revisao-por-pares.md`, fonte normativa da regua e dos papeis); o agente pode **montar o painel** sob acionamento humano (disparar os revisores diversos, colher vereditos — o despacho+coleta mecânico pode usar o harness `scripts/Invoke-LlmDelegatePanelDispatch.ps1`, que NÃO injeta nativo/piso/closeout/triagem/convergência —, parar no primeiro gap; piso: >=2 familias/criadores distintos via `targetModelKey` — um painel de uma voz nao e revisao por pares; subagente nativo conta pelo Criador de `targetModelKey`, nao pela ferramenta orquestradora (valide o piso sobre a lista COMPLETA de revisores, nativos inclusos); `ask` apresentados em lote, nunca descartados), mas a **decisão** de triagem de gaps, convergencia e push-ready e no humano — o agente **recomenda** sua triagem, mas não decide; a rotina por revisor continua sendo a do `13`
- modo assistido por dossiê (reforcada): nem todo revisor roda o passo mecanico. Revisor **git-capable** (Codex-delegate, subagente nativo) roda a rotina inteira do `13` e reproduz o git bruto; revisor **semantic-only** (sem shell: opencode `reviewer-ro`, Claude Code, Copilot, Gemini, Antigravity `public-review`) recebe um **dossiê** (`scripts/Build-PrePushReviewDossier.ps1`: Secao A git bruto + Secao B `-AsJson` rotulado «não verdade» + sentinelas) inline no prompt (stdin nos `stdin-dossier-capable` como o opencode; `argv-limited` Copilot/Gemini/Antigravity se couber, senão omitido) e roda só a fase semantica. Antigravity aceita apenas payload `public`, nunca e git-capable, recusa `-Cd` e usa scratch temporario validado; o gate roda primeiro, preserva `gateAsk`/`gateDeny` e somente um `allow` para `kb-sensitive` bloqueia antes do adapter como `unavailable` + `refusedSensitivity`. Seu recibo registra `cliVersion`, redirecionamento de ambiente e limpeza (`clean`/`degraded`/`failed`), com a limitacao explicita de que o keyring global nao e isolado. Painel valido retem >=1 git-capable que reproduz a Secao A; com working tree suja o orquestrador **não despacha** os semantic-only (pede commit/stash). Dono do modo: `13` («Modo assistido por dossiê»); política de painel: `14`
...
5. Em **cada nova sessao** na pasta paralela com override ativo, executar `Test-XpzCatalogOverrideSessionReminder.ps1 -ParallelKbRoot <raiz> -AsJson` e ler `status`, `noticeRequired`, `reminderRequired`, `cleanupRecommended`, `declaredUpstreamPending`, `effectiveUpstreamPending` e as listas `pendingTypeNames`/`redundantTypeNames`/`divergentTypeNames`. `CLEANUP_RECOMMENDED` significa que a pendencia efetiva acabou; em auditoria/setup, oferecer no plano consolidado a limpeza local aprovada das entradas redundantes, mesmo quando houver outro item obrigatório como recarimbo de assinatura. Não remover o arquivo nem a entrada sem aprovação.
...
- para tipos que permanecem `queryableByKbIntelligence=true` com grafo assimétrico (`API`, `DataSelector`, `WorkWith`, `WorkWithForWeb`, `ExternalObject`), ver `scripts/README-kb-intelligence.md` — `impact-basic` pode mostrar só saidas ou entradas esparsas; isso é esperado, não falha do índice; os dois Work With preservam identidades por GUID distintas mesmo quando nomes históricos forem ambíguos
...
- para separar objetos GERADOS por Pattern (WorkWith For Web) de AUTORAIS (ex.: auditoria de segurança), usar o sinal determinístico do índice (`is_generated_object`/`pattern_object_id`/`instance_key`/`instance_name` em `object-info`/`search-objects`/`list-by-type`; filtro `--generated`/`--authored`) em vez de heurística de nome (`ww*`/`*WC`/`*General`/`*Selection`); para "quais objetos a instância WorkWithForWeb X gerou", usar `search-objects -InstanceKey <nome|<GUID>-<nome>>` (auto-detecção nome vs chave completa, correspondência exata ignorando caixa; `-InstanceKey` só em `search-objects`, que passa a exigir `-ObjectName` OU `-InstanceKey`); o campo `instance_name` é derivado (`null` quando não derivável); a extração é escopada ao objeto raiz e não herda marcador de `<Object>` aninhado em `PackagedModule`; disponível a partir de `schema_version>=4`, com contador `generated_objects_count` na metadata; ver `02` e `scripts/README-kb-intelligence.md`
...
- delegar tarefa menor ou pedir segunda opiniao a um modelo secundario (skill [`xpz-llm-delegate`](xpz-llm-delegate/SKILL.md); ver lista e contrato dos backends suportados em `xpz-llm-delegate/SKILL.md`) só **a pedido do usuário ou com concordancia explicita dele**; nunca por iniciativa própria
...
- antes de enviar conteúdo a um modelo, classificar `kb-sensitive` (pasta paralela de KB) vs `public` (repo publico/molde sanitizado) e rodar `scripts/Resolve-LlmDelegateAuthorization.ps1` com o `-Backend` correspondente ao adapter (ver [`xpz-llm-delegate`](xpz-llm-delegate/SKILL.md)): `deny` não envia, `ask` exige autorizacao explicita do usuário, `allow` segue anunciando o destino ao usuário (campo `targetModelKey`)
- <!-- backend-parity: ignore --> preferir modelo local (loopback) para conteúdo de KB; conteúdo de pasta paralela só vai a modelo externo com autorizacao; a politica de confidencialidade casa sempre pela chave do provider de destino (ex.: `openai/*`, `anthropic/*`, `github-copilot/*`, `google/*`, `antigravity/*`), nunca pelo nome do adapter; adapters agenticos com permissao/sandbox/modo restritos **não** contornam o gate; ver `02-regras-operacionais-e-runtime.md` e [`xpz-llm-delegate`](xpz-llm-delegate/SKILL.md)
...
- no backend **opencode**, o revisor roda por padrao no agente **`reviewer-ro`** least-privilege («sem execucao/escrita»): `Invoke-OpenCode.ps1`/`Start-OpenCodeJob.ps1` tem **default `-Agent reviewer-ro`** + **guard fail-closed** (pre-check estatico + `opencode agent list` allow-set resolvido `{read,grep,glob,list}` + versao testada, ANTES do run/spawn; BLOCK com motivo se divergir). Nega `bash`/`edit`/`webfetch`/`websearch`/`task`; `external_directory[*]=deny` bloqueia por padrao leitura fora do cwd herdado, embora os fixtures 1.18.30 (ver `xpz-llm-delegate/fixtures/opencode-reviewer-ro/VERSION.txt`) tragam exceções internas do opencode — isso **inverte a premissa antiga** de que a tool `read` do opencode «lia qualquer arquivo da máquina» (refutada por medição promovida em 1.18.30: a base `external_directory: ask` é auto-rejeitada em headless). `-Agent <x>` explicito e opt-out consciente (o chamador assume a postura). Isso e ORTOGONAL ao gate de confidencialidade (que governa o **envio** do dado). **cwd-seguro e operacional:** quem dispara escolhe um cwd sem segredos nao-versionados; `.env` dentro do cwd é risco alto pendente no `999`. Ver `xpz-llm-delegate/SKILL.md` e `15-revisao-por-pares.md`

### Consumo de resultado assíncrono opencode

Português (BR): ao usar `Start-OpenCodeJob.ps1`, o JSON inicial contém identidade do job, PID do runner `pwsh` e caminhos; isso não é aceite de parecer nem PID direto do opencode. Para aceitar resultado assíncrono opencode, aguarde `<GUID>.result.json`, valide pelo helper compartilhado de `OpenCodeStreamSupport.ps1` e use apenas `acceptedFinalText` quando `schemaVersion=2`, o shape v2 for válido, `resultAccepted=true`, `status=completed`, `hasStepFinish=true`, `completionVerdict=ok`, `finishReason=stop`, `watcherExitCode=0`, `opencodeExitCode=0`, `fallbackToBuild=false`, `fallbackDetail=null`, `finalTextDisposition=accepted`, `error=null`, `rejectionReason=''` e `acceptedFinalText` não vazio e igual a `finalText`. Não aceite por `finalText` existir, por `status` isolado, por `finishReason` isolado, por `completionVerdict=ok` sem `hasStepFinish=true`, por `ExitCode=0` isolado, nem pelo campo `result` devolvido por `Start-OpenCodeJob.ps1`. Com `-NoWatcher`, `result` é só caminho previsto e pode não existir até um watcher ser executado — e **não** há detecção de limite de uso/taxa sem watcher. `-WatchTimeoutSec` (default 0) exige watcher se > 0. Anexação depois de `-NoWatcher`: passar `-ExpectedStartTimeUtc` (ISO-Z do StartTime do runner). Rejeição controlada do watcher usa `watcherExitCode=20` (inclui `limite-uso`, `limite-taxa` e `opencode-watch-timeout`); recusa de clobber usa `21`; falha interna do próprio watcher antes da promoção atômica usa código `99`, emite `WATCHER_INTERNAL_ERROR` e não promove `<GUID>.result.json`. Se o shape mínimo v2 for inválido, classifique como contrato corrompido ou insuficiente, não como parecer aceito. O helper também recusa resultado rejeitado com coerência interna corrompida ou fabricada. Campos opcionais do v2 (`sinceTimeSource`, trio de identidade, `watchedProcessStillAliveAtPromote`), se presentes e inválidos, fazem o helper recusar **antes** do aceite (`invalid-sinceTimeSource`, `rejected-identity-incoherent`, `invalid-watchedProcessStillAliveAtPromote`). Detalhe no dono `xpz-llm-delegate/SKILL.md`. No caminho síncrono do painel, limite de uso e de taxa compartilham o circuito `quota`; jobs não passam por esse circuito.

Español: al usar `Start-OpenCodeJob.ps1`, el JSON inicial contiene identidad del job, PID del runner `pwsh` y rutas; eso no acepta el dictamen ni es el PID directo de opencode. Para aceptar un resultado asíncrono de opencode, espere `<GUID>.result.json`, valide con el helper compartido de `OpenCodeStreamSupport.ps1` y use solo `acceptedFinalText` cuando `schemaVersion=2`, el shape v2 sea válido, `resultAccepted=true`, `status=completed`, `hasStepFinish=true`, `completionVerdict=ok`, `finishReason=stop`, `watcherExitCode=0`, `opencodeExitCode=0`, `fallbackToBuild=false`, `fallbackDetail=null`, `finalTextDisposition=accepted`, `error=null`, `rejectionReason=''` y `acceptedFinalText` no esté vacío y sea igual a `finalText`. No acepte por existir `finalText`, por `status` aislado, por `finishReason` aislado, por `completionVerdict=ok` sin `hasStepFinish=true`, por `ExitCode=0` aislado, ni por el campo `result` devuelto por `Start-OpenCodeJob.ps1`. Con `-NoWatcher`, `result` es solo una ruta prevista y puede no existir hasta que se ejecute un watcher — y **no** hay detección de límite de uso/tasa sin watcher. `-WatchTimeoutSec` (default 0) exige watcher si es > 0. Al anexar el watcher después de `-NoWatcher`: pasar `-ExpectedStartTimeUtc` (ISO-Z del StartTime del runner). El rechazo controlado del watcher usa `watcherExitCode=20` (incluye `limite-uso`, `limite-taxa` y `opencode-watch-timeout`); la recusa de clobber usa `21`; una falla interna del propio watcher antes de la promoción atómica usa código `99`, emite `WATCHER_INTERNAL_ERROR` y no promueve `<GUID>.result.json`. Si el shape mínimo v2 es inválido, clasifíquelo como contrato corrupto o insuficiente, no como dictamen aceptado. El helper también rechaza resultados rechazados con coherencia interna corrupta o fabricada. Campos opcionales del v2 (`sinceTimeSource`, trío de identidad, `watchedProcessStillAliveAtPromote`), si están presentes e inválidos, hacen que el helper rechace **antes** de la aceptación (`invalid-sinceTimeSource`, `rejected-identity-incoherent`, `invalid-watchedProcessStillAliveAtPromote`). Detalle en el dueño `xpz-llm-delegate/SKILL.md`. En el camino síncrono del panel, límite de uso y de tasa comparten el circuito `quota`; los jobs no pasan por ese circuito.

English: when using `Start-OpenCodeJob.ps1`, the initial JSON contains job identity, the `pwsh` runner PID, and paths; it is not acceptance of the review and not the direct opencode PID. To accept an asynchronous opencode result, wait for `<GUID>.result.json`, validate it through the shared helper in `OpenCodeStreamSupport.ps1`, and use only `acceptedFinalText` when `schemaVersion=2`, the v2 shape is valid, `resultAccepted=true`, `status=completed`, `hasStepFinish=true`, `completionVerdict=ok`, `finishReason=stop`, `watcherExitCode=0`, `opencodeExitCode=0`, `fallbackToBuild=false`, `fallbackDetail=null`, `finalTextDisposition=accepted`, `error=null`, `rejectionReason=''`, and `acceptedFinalText` is not empty and equals `finalText`. Do not accept because `finalText` exists, because of `status` alone, `finishReason` alone, `completionVerdict=ok` without `hasStepFinish=true`, `ExitCode=0` alone, or the `result` field returned by `Start-OpenCodeJob.ps1`. With `-NoWatcher`, `result` is only the expected path and may not exist until a watcher runs — and there is **no** usage/rate-limit detection without a watcher. `-WatchTimeoutSec` (default 0) requires a watcher when greater than 0. Attach after `-NoWatcher`: pass `-ExpectedStartTimeUtc` (runner StartTime ISO-Z). Controlled watcher rejection uses `watcherExitCode=20` (including `limite-uso`, `limite-taxa`, and `opencode-watch-timeout`); clobber refusal uses `21`; internal watcher failure before atomic promotion uses code `99`, emits `WATCHER_INTERNAL_ERROR`, and does not promote `<GUID>.result.json`. If the minimum v2 shape is invalid, classify it as a corrupt or insufficient contract, not as an accepted review. The helper also rejects rejected results whose internal fields are corrupt or fabricated. Optional v2 fields (`sinceTimeSource`, identity trio, `watchedProcessStillAliveAtPromote`), if present and invalid, make the helper reject **before** acceptance (`invalid-sinceTimeSource`, `rejected-identity-incoherent`, `invalid-watchedProcessStillAliveAtPromote`). Detail lives in the owner `xpz-llm-delegate/SKILL.md`. On the synchronous panel path, usage and rate limits share the `quota` circuit; jobs do not go through that circuit.
...
- para o unico par declarativo hoje reconhecido, `Transaction` + `Rules`, o gate suprime somente os falsos findings procedurais `unexpected-close`, `mismatched-close` e `unclosed-block` que podem surgir de clauses `if` multiline ou blocos `[web]`; os warnings permanecem e `Events`, `Procedure`, demais pares e `Conditions` seguem no balanceamento procedural
...
- o resultado do sync é uma **linha JSON de máquina no stdout** (`Kind=xpz-sync-result`, `SchemaVersion=1`, `-Compress`); todo texto humano sai no stderr. Consumir via `ConvertFrom-Json`, não parsear o texto. Em processo filho (`pwsh -File`), `Write-Host`/`Write-Warning`/`Write-Information` vazam para o stdout capturado — por isso o contrato exige stderr para diagnóstico
...
- detecção cross-fluxo de origem (`dataSource`): o resultado traz `CrossFlowCollisions` (array; `[]` vazio quando não há) — colisão moderno↔legado no mesmo `FolderType|NormalizedName` (sobrescrita silenciosa que deixou de ser silenciosa); `Writes[].CrossFlowCollision` no `$report`. Exemplo mínimo: `"CrossFlowCollisions":[]` no caso normal; com colisão, um objeto `{FolderType, NormalizedName, ExistingDataSource, IncomingDataSource, …}`. Switch `-BlockCrossFlowDataSource` aborta o sync em vez de só sinalizar. `SchemaVersion` permanece `1` (campos aditivos). Ressalva: a atomicidade pré-metadata **não** vale para o fail-closed de rename sob `-FullSnapshot` (aborta pós-metadata)
...
- tratar `metadata/deploy` diferente de `OK` como pendencia metodologica real: `metadata wrapper: OK` não prova lista correta de environments nem mapeamento de output/web; em `java-tomcat`, também não prova `kb_environment_servlet_dirs`, `kb_environment_app_package` nem `kb_environment_servlet_flavor`. Perguntar nomes e diretórios de output ao usuário, usar `Resolve-XpzJavaTomcatMetadataSuggestion.ps1` apenas como assistente read-only quando aplicavel, gravar com `-KbEnvironmentNames`, `-KbEnvironmentOutputDirs` e campos Java opt-in confirmados, e validar via MSBuild; corrigir wrapper local se `INVENTORY_CUSTOMIZED` apontar `uses_removed_inventory_discovery` ou parametro obrigatório de environment/output ausente; quando `INVENTORY_SURFACE_ADVISORY` apontar ausência de parâmetro Java/Tomcat opt-in, tratar como realinhamento consultivo de superfície, não como bloqueio automático
...
- `git diff --check` não autoriza limpeza global de whitespace, trailing spaces, indentação ou EOL no XML inteiro; corrija apenas linhas novas/editadas e preserve whitespace herdado do acervo fora do delta funcional aprovado
...
- para alterar **metadados** de muitos XMLs da frente na mesma rodada (documentação do Part `babf62c5-…`, `parent`/`parentGuid`/`parentType`, rename de `Domain`), usar `scripts\Edit-GeneXusXmlBatchMetadata.ps1` com manifesto JSON (`Kind=xpz-batch-metadata-manifest`, `SchemaVersion=1`) em vez de script descartável por frente ou de repetir o motor cirúrgico por arquivo; o manifesto declara `guid`, tipo, nome e precondição por operação, e o motor confere cada declaração contra o arquivo real antes de gravar; rodar primeiro **sem** `-Apply` (o plano não deixa artefato do motor: journal, `.bak`, baseline, temporários e o `-WorkDir` que ele tenha criado; o relatório de `-ReportPath`, por ser saída pedida pelo chamador num caminho que ele deu, é gravado e é a única coisa que fica) e só depois com `-Apply`, que materializa journal e `.bak` em `-WorkDir` fora da frente; `checksum` é preservado e o resultado vira `checksumStale`, nunca `ok` puro; após interrupção, ler o journal com `scripts\Show-GeneXusXmlBatchMetadataRecoveryPlan.ps1` antes de mexer à mão; ver `edit-genexus-xml-batch-metadata-design.md`
...
- em resposta ao usuário, separar explicitamente `sucesso operacional da chamada MSBuild`, `preview sem alteracao real da KB`, `sucesso operacional com falha no pos-processamento` (MSBuild concluiu com `executionEvidence.msBuildExitCode=0` e a evidencia primaria do log bruto está presente — import/export: `__IMPORTED_ITEM__` ou `__EXPORTED_FILE__` mais arquivo XPZ existente; build-all: `__KB_OPEN__=true` **e** `__BUILDALL_DONE__=true`, materializados como `observedContext.KbOpen=true` **e** `observedContext.BuildAllDone=true`; specify-generate: `__SPECIFY_DONE__=true` **e** `__GENERATE_DONE__=true`, materializados como `observedContext.SpecifyDone=true` **e** `observedContext.GenerateDone=true` —, mas o pos-processamento local do wrapper falhou e o JSON saiu com `postProcessingFailed=true`; não reclassificar como `falha operacional` nem exit 90 quando a task MSBuild estiver limpa), `preview apenas com falha no pos-processamento` (analogo na fase de preview: `executionEvidence.msBuildExitCode=0` sem alterar a KB, evidencia primaria preservada no log bruto, mas pos-processamento local falhou) e `confirmacao funcional pendente na IDE oficial`
- no recovery externo de `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1`, só interpretar `compilou limpo com falha no pos-processamento` quando o JSON parcial preservar `executionEvidence.msBuildExitCode=0`, classificação anterior limpa e `observedContext.KbOpen=true` + `BuildAllDone=true`; só interpretar `specify e generate concluídos com falha no pos-processamento` quando preservar classificação anterior limpa e `SpecifyDone=true` + `GenerateDone=true`. Com Categoria B (`exitCode=48`), reorg, timeout, MSBuild não-zero ou marcador ausente, manter `status`/`exitCode` classificados e usar `executionEvidence.StdOutPath`/`StdErrPath` e `artifacts` para a evidência; nunca normalizar para sucesso limpo nem assumir `exitCode=90` por o diagnóstico estar degradado.
...
- em `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1`, quando `stdoutSignals.postBuildEvents` vier com itens (eventos pos-build configurados na KB que dispararam ou tentaram disparar processo externo), eles são capturados pela janela `Executando eventos pos-construcao ...` ate o próximo separador `==========` (fallback `start c:` / `start cmd` em logs sem marcador) e **classificados** contra `kb_environment_post_build_event_hashes` do environment ativo em `kb-source-metadata.md`, com o veredito agregado em `stdoutSignals.postBuildEventClassification`: duração `TimeSpan` válida e data civil válida são descartadas na extração da janela e não aparecem como eventos; `REM` comentado pode aparecer como item `(commented)` e é classificado como `inert`, sem rebaixar. Comando de timing/marcador e demais linhas operacionais da janela entram na classificação, exceto diagnósticos estruturais descartados pelo motor (`(N,N): warning|error:`, `Build succeeded`/`Build FAILED`, `Compilación`/`Compilação`). Evento **registrado** = esperado (informativo, **não** rebaixa); **não registrado/nao reconhecido** = rebaixa por cautela; sem registro para o environment, player de som (`SoundPlayer`/`PlaySync`/`.wav`) e reconhecido como benigno e o resto rebaixa. O fingerprint registrado para a saída prevista do contorno `Verify-GxJs` com `invalidos: 0` ignora apenas a variação de `tempo: <n> ms`, preservando contagens, raiz e demais campos. Só rebaixar para `operacao concluida, pendente de confirmacao funcional` quando houver evento que rebaixa (`postBuildEventClassification.shouldDowngrade=true`) — nesse caso citar o evento e orientar inspecao de `stdoutSignals.postBuildEventClassification`. Evento legitimo ainda não registrado deve ser registrado via `xpz-kb-parallel-setup` (`Register-GeneXusKbPostBuildEvents.ps1`), após confirmacao do usuário, para deixar de rebaixar
...
- quando o XML autoritativo já está na pasta paralela, **não** exportar da KB só para obter casca de `.xpz`; preferir `import_file.xml` montado via `xpz-builder` (`Build-GeneXusImportFileEnvelope.ps1` com `-AcervoPath <ObjetosDaKbEmXml>` obrigatório, coerencia pré-gravação por `lastUpdate` e `Object/@type` por mesmo `guid`, e objetos modificados declarados por `-ModifiedObjectNames` ou `-ModifiedObjectGuids`) ou pelo fluxo de frente com a entrada segura `New-XpzImportPackage.ps1`; para pacote misto/complexo, preferir `-TemplatePackagePath` com pacote real comparavel, salvo pedido explicito do usuário ou confirmacao de que o envelope não pode ser montado por outro meio documentado
...
- em KB com `kb_environment_count` > 1 em `kb-source-metadata.md`, build de validação pós-import exige `-ParallelKbRoot` (ou `-KbMetadataPath`), `-EnvironmentName` ou `deployment_environment_name` gravado pelo `xpz-kb-parallel-setup`; lista de environments e mapeamento de output/web no setup vêm do usuário (`-KbEnvironmentNames`, `-KbEnvironmentOutputDirs`, `kb_environment_web_dirs`) com validação MSBuild obrigatória — não scan de pastas; não confundir `compilou limpo` no environment ativo da IDE com deploy no IIS; usar `-PostImportDeployValidation` para gate de publicacao no destino de deploy (.NET: `web\bin`; `java-tomcat`: co-gate `WEB-INF\classes` externo) — exit **49** sem evidencia fresca; `GxNetCoreStartup.dll` velho em incremental e warning, não gate
- ao ler o JSON de `Invoke-GeneXusKbBuildAll.ps1`/`Invoke-GeneXusKbSpecifyGenerate.ps1` com `-PostImportDeployValidation`/`-StrictDeployBinCheck`, interpretar os sinais de deploy-bin (produzidos por `Invoke-GeneXusKbDeployBinPostBuildClassification` em `scripts\GeneXusKbDeployBinSupport.ps1`): `deployBinFreshness` (`fresh`/`stale`/`unknown`/`skipped-hosting-unsupported`; **+ para `java-tomcat`**: `no-evidence`/`unexpected-publication`), `deployBinCheck.binCheck` (`publicationFreshSinceBuild`, `objectDllMaxWriteTime`, `configMaxWriteTime`) e `deployBinCheck.interpretation` (irmão de `binCheck`, não filho); **nunca** declarar deploy OK com `deployBinFreshness=stale`, `publicationFreshSinceBuild=false`, `exit 49` ou status `compilou-mas-dll-destino-desatualizada` — investigar `deployBinCheck.interpretation` e os timestamps de DLL/config; `GxNetCoreStartup.dll` velho sozinho em incremental e warning de sentinela Core, não gate. **Paridade Java/Tomcat (Fase 3, PER-EIXO):** o **Eixo A (deploy-bin) de `java-tomcat` RODA** (co-gate por conjunto de artefatos no `WEB-INF\classes` externo) — `deployBinFreshness` pode ser `fresh`/`stale`/`no-evidence`/`unexpected-publication`/`unknown`; `stale`/`unexpected-publication`/`unknown` falham sob gate (exit 49), `no-evidence` (build sem mudança) **não** falha mas **não** é deploy OK. Só os Eixos B/C (`.cs`/`.java`, runtime) seguem `skipped-hosting-unsupported`/`recognized-no-engine` (Pós-v1) — **skip, não deploy validado**. No caminho Java o `deployBinCheck.binCheck` tem **shape próprio** (`rule`/`localObjectsFresh`/`publishedObjectsFresh`/`publishedAboveUpperWindow`), **não** os campos .NET (`publicationFreshSinceBuild`/`objectDllMaxWriteTime`/`configMaxWriteTime`); a âncora superior da janela sai em `deployBinCheck.thresholdSupAt` (Fase 3 = `Now`). Detalhe normativo em `xpz-msbuild-build/SKILL.md` e tabela per-eixo em `02-regras-operacionais-e-runtime.md`
- ao usar a checagem de frescor de runtime (`scripts\Test-GeneXusRuntimeFreshness.ps1`) em KB `java-tomcat`, passar `-DeploymentHostingKind java-tomcat`: o Eixo C ainda é `recognized-no-engine`/Pós-v1 e deve retornar `skipped-hosting-unsupported`/exit 0. Não concluir pendência de runtime Java por `ObjStatus=genreq` isolado; a evidência empírica exige cruzar `ObjNavig`, XMLs de navegação/specification e artefatos `.java`/`.class`/`.js` em frente futura
- para diagnosticar `.cs` gerado, usar `scripts\Resolve-GeneXusGeneratedCsPath.ps1` a partir de `kb-source-metadata.md` (`kb_environment_web_dirs`) antes de chamar `Find-CsAttributeAssignments.ps1` ou buscar handler de `WebPanel`; se o mapeamento estiver ausente, bloquear e atualizar setup via `xpz-kb-parallel-setup`, sem glob recursivo em `C:\GxModels` (exceção Fase 2: KB `java-tomcat`/família não-.NET pula com `CS_PATH_SKIPPED_HOSTING_UNSUPPORTED`/exit 0 antes desse bloqueio — guarda de família do Eixo B)
...
- Regra operacional: quando o wrapper receber `-ReportPath <caminho-absoluto.json>`, o relatório `Kind=xpz-package-execution-report`, `SchemaVersion=1` deve ser lido antes de concluir a rodada: `executionState=completed` não basta sem `packageState=accepted` e `inventoryDecision=accepted`; `candidate`, `unknown` ou `running` exigem revisão/retomada. O arquivo é publicado atomicamente em UTF-8 sem BOM, com stdout/stderr separados e `stageHistory` limitado; ele não constitui prova de importação real, build, IDE ou comportamento funcional. Caminho inválido bloqueia antes de qualquer artefato. Com `-ReportPath` válido, o pós-inventário liga fail-closed (`FailOnDeltaMismatch` + `FailOnUnknownTypes`) e pode mudar o exit do empacotamento (`status=bloqueado`); sem `-ReportPath`, o inventário é informativo e não promove esse exit.
...
- quando houver YAML OpenAPI gerado pós-build/deploy, usá-lo apenas como evidência complementar do contrato público observado (arquivo/ambiente/frescor declarados) e manter separadas a fonte XML/XPZ, o contrato publicado no YAML e a segurança provada por runtime/smoke HTTP; ver `xpz-builder/responsibilities-by-type/api.md`.
...
- para empacotamento com `Build-GeneXusImportFileEnvelope.ps1`, informar obrigatoriamente `-AcervoPath <ObjetosDaKbEmXml>`; o script sempre executa coerencia frente-vs-acervo antes de gravar: gate de `lastUpdate` e bloqueio de `Object/@type` divergente quando o XML de entrada e o acervo têm o mesmo `guid`; o agente deve informar `-ModifiedObjectNames` ou `-ModifiedObjectGuids` para que o script bloqueie `lastUpdate` velho, igual ao acervo em objeto modificado ou futuro demais antes de escrever o pacote; para `Panel`, o helper repassa automaticamente `-TemplatePackagePath` como `-PanelReferencePath` e propaga `information`/`warnings` do gate de par `level id`/`layout id`; ele continua cobrindo coerencia documental de `lastUpdate` e `Object/@type` conforme seu contrato atual, mas não cobre trim global forte nesta versão
- para empacotamento por frente, use `New-XpzImportPackage.ps1` como entrada operacional segura; o `.py` é motor interno e não é rota equivalente porque não executa sozinho as validações prévias. O gate de drift 9-FD (`Test-GeneXusFrontAcervoDrift.ps1`) executa sempre antes do empacotamento (fail-closed) e bloqueia XMLs na frente com `lastUpdate` mais antigo que o homônimo no acervo, com mesmo `guid` e `Object/@type` divergente (`front-object-type-drift`) ou com remoção forte de trailing whitespace herdado contra baseline por GUID único; proteção mecânica parcial nesta versão: o 9-FD não detecta introdução de trailing whitespace novo e não é detector completo de whitespace, reindentação ou EOL. `-AcervoPath <ObjetosDaKbEmXml>` é opcional e, quando omitido, o acervo canônico `<RepoRoot>/ObjetosDaKbEmXml` é resolvido automaticamente; sem acervo resolvível o empacotamento é bloqueado, e o JSON reporta `acervoResolvedBy` (`explicit` ou `convention`). Payload novo de type drift usa `objectGuid` normalizado, `matchBasis`, `acervoPath`/`candidateAcervoPaths` e `message`; acervo ambíguo por GUID sai como `front-object-type-drift-ambiguous-acervo` informativo, sem escolher tipo oficial. Payload textual público do bloqueio usa `classification`, `whitespaceTrimPairCount`, `functionalChangedLineEstimate`, `acervoLineCount` e `thresholds`; o finding informativo textual usa `classification='not-applicable'` e `reason`. Findings `front-older-than-acervo`, `front-object-type-drift` e `front-textual-fidelity-trim-removal-churn` (`fail`) bloqueiam o empacotamento; findings `front-equals-acervo` e `lastupdate-unparseable` (`warn`) exigem confirmação explícita; `front-textual-fidelity-info` é informativo apenas quando havia baseline único por GUID, mas a leitura/decodificação UTF-8 comparável falhou; colisao de rodada retorna JSON com `status=bloqueado`, `reason=PACKAGE_ROUND_COLLISION`, `blockingReasons` e `nextFreeNN`; resolução de drift de `lastUpdate`: usar `Copy-GeneXusAcervoToFront.ps1` para copiar do acervo para a frente com bump automático de `lastUpdate`; resolução de bloqueio textual: chamar o mesmo script com alvo explícito (`-ObjectList`, `-ObjectNames` ou `-ObjectGuids`) para reconstruir a cópia mesmo quando a frente está mais nova, reaplicar só o delta funcional, depois re-executar o gate 9-FD; alvo explícito sobre objeto já existente/editado pode sobrescrever a cópia local e deve ser deliberado, preferencialmente com `-DryRun`; resolução de `front-object-type-drift`: decisão humana, sem autocopiar/bump, para determinar se o XML correto é o da frente, o do acervo ou se houve erro de geração/seleção; para seed inicial de objeto que ainda não existe na frente, chamar o mesmo script com `-ObjectList`, `-ObjectNames` ou `-ObjectGuids` explicito; a pasta da frente deve já existir, aberta por `New-GeneXusXpzFront.ps1` (wrapper `New-*KbFront.ps1`, `-ReuseIfExists` para retomar), nunca criada manualmente — `Copy-GeneXusAcervoToFront.ps1` e os gates 9-* apenas populam ou inspecionam uma frente existente
- editar `ObjetosDaKbEmXml` esperando que o pacote use essa versão é anti-padrão; o motor de empacotamento lê da pasta da frente, nunca do acervo; o gate 9-FD detecta drift de `lastUpdate`, drift de `Object/@type` por mesmo `guid` e remoção forte de trailing whitespace herdado contra baseline por GUID único, mas o anti-padrão conceitual permanece mesmo quando esses sinais não aparecem; para alterar um objeto, copiar do acervo para a frente, editar a cópia e bump o `lastUpdate` — nunca editar o acervo diretamente
...
Quando a alteração textual já estiver aprovada, use normalmente a ferramenta nativa `apply_patch` do Codex, desde que nenhuma instrução de maior precedência a proíba. Antes da escrita, mostre o diff completo, delimite arquivos/regiões e aguarde aprovação explícita; depois valide `git status --porcelain`, diff, `git diff --check`, encoding/EOL e releitura do trecho aplicável.

Use xpz-codex-apply-patch-alternative/SKILL.md como backup auditável quando a rota nativa estiver indisponível, falhar antes de qualquer escrita local com árvore/índice sem modificação, ou houver instrução humana expressa. Quando a skill estiver disponível por registro global, symlink, junction ou caminho publicado equivalente, o backup pode operar sobre outro repositório alvo por `-RepositoryRoot`; esse parâmetro deve apontar para a raiz de uma work tree Git não-bare na branch `main`. Não presuma que o script exista localmente em `scripts/` no repositório alvo. Nessa rota, `pwsh -File` recebe argv literal, mesmo quando o chamador o monta com ProcessStartInfo.ArgumentList, e não associa `-AllowedPath` repetido ao parâmetro array. A falha ocorre antes do motor: não há JSON no stdout e o diagnóstico do binder vem no stderr. Envie um patch e um -AllowedPath por ciclo; transmita o Base64 contíguo ao -DryRun com StandardInput.Write(...) seguido de StandardInput.Close(), sem WriteLine, pipe ou whitespace. Para vários arquivos, repita os ciclos independentes de stage -> apply, sem atomicidade entre os já aplicados. Confirme stagedPatchId/patchSha256 no JSON e aplique com ambos. O motor não substitui a validação posterior nem autorização humana; ele não faz commit, push, checkout, reset ou rollback.
