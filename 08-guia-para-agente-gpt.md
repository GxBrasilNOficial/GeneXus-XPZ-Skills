...
- para separar objetos GERADOS por Pattern (WorkWith For Web) de AUTORAIS (ex.: auditoria de segurança), usar o sinal determinístico do índice (`is_generated_object`/`pattern_object_id`/`instance_key` em `object-info`/`search-objects`/`list-by-type`; filtro `--generated`/`--authored`) em vez de heurística de nome (`ww*`/`*WC`/`*General`/`*Selection`); a extração é escopada ao objeto raiz e não herda marcador de `<Object>` aninhado em `PackagedModule`; disponível a partir de `schema_version>=4`, com contador `generated_objects_count` na metadata; ver `02` e `scripts/README-kb-intelligence.md`
