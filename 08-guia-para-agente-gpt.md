...
- para alterar **metadados** de muitos XMLs da frente na mesma rodada (documentação do Part `babf62c5-…`, `parent`/`parentGuid`/`parentType`, rename de `Domain`), usar `scripts\Edit-GeneXusXmlBatchMetadata.ps1` com manifesto JSON (`Kind=xpz-batch-metadata-manifest`, `SchemaVersion=1`) em vez de script descartável por frente ou de repetir o motor cirúrgico por arquivo; o manifesto declara `guid`, tipo, nome e precondição por operação, e o motor confere cada declaração contra o arquivo real antes de gravar; rodar primeiro **sem** `-Apply` (o plano não deixa artefato persistente) e só depois com `-Apply`, que materializa journal e `.bak` em `-WorkDir` fora da frente; `checksum` é preservado e o resultado vira `checksumStale`, nunca `ok` puro; após interrupção, ler o journal com `scripts\Show-GeneXusXmlBatchMetadataRecoveryPlan.ps1` antes de mexer à mão; ver `edit-genexus-xml-batch-metadata-design.md`
