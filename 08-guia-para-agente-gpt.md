...
- no backend **opencode**, o revisor roda por padrao no agente **`reviewer-ro`** least-privilege («sem execucao/escrita»): `Invoke-OpenCode.ps1`/`Start-OpenCodeJob.ps1` tem **default `-Agent reviewer-ro`** + **guard fail-closed** (pre-check estatico + `opencode agent list` allow-set resolvido `{read,grep,glob,list}` + versao testada, ANTES do run/spawn; BLOCK com motivo se divergir). Nega `bash`/`edit`/`webfetch`/`websearch`/`task`; a leitura fica **confinada ao cwd herdado** (`external_directory: deny`) — isso **inverte a premissa antiga** de que a tool `read` do opencode «lia qualquer arquivo da máquina» (refutada por medição em 1.4.4: a base `external_directory: ask` é auto-rejeitada em headless). `-Agent <x>` explicito e opt-out consciente (o chamador assume a postura). Isso e ORTOGONAL ao gate de confidencialidade (que governa o **envio** do dado). **cwd-seguro e operacional:** quem dispara escolhe um cwd sem segredos nao-versionados. Ver `xpz-llm-delegate/SKILL.md` e `15-revisao-por-pares.md`
