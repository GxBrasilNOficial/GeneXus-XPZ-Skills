...
- ao ler o JSON de `Invoke-GeneXusKbBuildAll.ps1`/`Invoke-GeneXusKbSpecifyGenerate.ps1` com `-PostImportDeployValidation`/`-StrictDeployBinCheck`, interpretar os sinais de deploy-bin (produzidos por `Invoke-GeneXusKbDeployBinPostBuildClassification` em `scripts\GeneXusKbDeployBinSupport.ps1`): `deployBinFreshness` (`fresh`/`stale`/`unknown`/`skipped-hosting-unsupported`), `deployBinCheck.binCheck` (`publicationFreshSinceBuild`, `objectDllMaxWriteTime`, `configMaxWriteTime`) e `deployBinCheck.interpretation` (irmão de `binCheck`, não filho); **nunca** declarar deploy OK com `deployBinFreshness=stale`, `publicationFreshSinceBuild=false`, `exit 49` ou status `compilou-mas-dll-destino-desatualizada` — investigar `deployBinCheck.interpretation` e os timestamps de DLL/config; `GxNetCoreStartup.dll` velho sozinho em incremental e warning de sentinela Core, não gate. **Paridade Java/Tomcat (Fase 2):** `deployBinFreshness=skipped-hosting-unsupported` (exit 0) significa hosting kind reconhecido **sem motor** (`java-tomcat`); é **skip, não deploy validado** — não declarar deploy OK sobre um skip. Detalhe normativo em `xpz-msbuild-build/SKILL.md` e tabela estado→status em `02-regras-operacionais-e-runtime.md`
